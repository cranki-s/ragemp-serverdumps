...
//     if (alt) {
//         if(animationSlots) NewEvent.callRemote('playAnim', animationSlots["1"][0].ad,animationSlots["1"][0].an,animationSlots["1"][0].af); 
//     }
// });
...
mp.events.add('Animations.FastSlot.Use', (slotId)=>{
    ...
    { 
        ...
        NewEvent.callRemote('playAnim', animationSlots[slotId][0].ad, animationSlots[slotId][0].an, animationSlots[slotId][0].af);
    }
})
    ...
    try{
        for(let i = 49; i<=54;i++)
        {
            ...
            if (x&&y) {
                ...
                { 
                    mp.events.call('Hud.InfoButtons.Add', JSON.stringify(['C']), 'Остановить анимацию');
                    NewEvent.callRemote('playAnim', animationSlots[i-48][0].ad, animationSlots[i-48][0].an, animationSlots[i-48][0].af);
                }
            }
        }
    }catch{}
...
mp.events.add('playAnim',(animation)=>{    
    ...
    NewEvent.callRemote('playAnim', animation.ad,animation.an,animation.af);
    global.isPlayerUseAnim = true;
})
mp.events.add('setWalk',(animation)=>{
    animation = JSON.parse(animation)
    NewEvent.callRemote('aSelected', 12, animation.style);
})
