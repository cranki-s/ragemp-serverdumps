...
mp.events.add('openTun', (businessMarkup, carClassMarkup, carComponents, businessType,playerCash,playerBank) => {
    ...
    NewEvent.callRemote('EnterTuningMenu');
...
mp.events.add('Tuning:buyItems', function (type,components) {
    ...
    NewEvent.callRemote('Tuning:BuyItems', type, JSON.stringify(components),types);
...
mp.events.add('tuningSeatsCheck', function (type) {
    ...
    NewEvent.callRemote('tuningSeatsCheck', type);
});
...
mp.events.add('Tuning:closeApp', function () {
    ...
    carCategories = [];
    if (lsc != null) {
        NewEvent.callRemote('exitTuning');
        lsc.destroy();
        lsc = null;
    }
    global.menuClose();
