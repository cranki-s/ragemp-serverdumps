...
mp.events.add('Bank:AccountInteraction',(accType,type,sum)=>{
    Nexus.callRemote('Bank:AccountInteraction',accType,type,sum)
})
mp.events.add('Bank:AccountTransfer',(account,sum)=>{
    Nexus.callRemote('Bank:AccountTransfer',account,sum)
});
mp.events.add('Bank:TaxesTransfer',(type,sum)=>{
    Nexus.callRemote('Bank:TaxesTransfer',type,sum)
});
