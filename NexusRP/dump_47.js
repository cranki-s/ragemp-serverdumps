...
mp.events.add('clShop:GetItems',(shopType,category)=>{
    ...
    //NewEvent.callRemote('console',JSON.stringify(items));
...
mp.events.add('clShop:BuyItem', (MoneyType, shopType, activeCategory, activeItem, activeModification) => {
    ...
    if(shopType =="backpacks"){
        NewEvent.callRemote('backsbuy', item.Variation, item.color,MoneyType);    
    }
    else if(shopType =="masks"){
        NewEvent.callRemote('buyMasks', item.Variation, item.color[activeModification.id],MoneyType);
    }
    else if(shopType =="tattoo"){
        let gender = localplayer.getVariable("GENDER");
        let hash = gender ? item.MaleHash : item.FemaleHash;
        NewEvent.callRemote("buyTattoo", hash, item.price, MoneyType, activeCategory.id,JSON.stringify(item.Slots),item.Dictionary,item.name);
    }
    else if(shopType =="appearance"){
        NewEvent.callRemote("buyBarber", activeCategory.type, barberValues[activeCategory.type].Style, barberValues[activeCategory.type].Color,MoneyType);
    }
})
...
mp.events.add('clShop:CloseBrowser',()=>{
    ...
    if(global.QuestShop)
    {
        NewEvent.callRemote('QuestShop:Close');
        return;
    }
    ...
    if(StoreType=="backpacks"){
        NewEvent.callRemote('closebacks');
    }
    else if(StoreType=="masks"){
        NewEvent.callRemote('cancelMasks');
    }
    else if(StoreType=="tattoo" || StoreType == "appearance"){
        localplayer.clearDecorations();
        NewEvent.callRemote("cancelBody");
    }    
});
