...
mp.keys.bind(Keys.VK_R, false, function () { // R key
	try {
		...
		NewEvent.callRemote("playerReload", current, ammo);
	} catch { }
});
...
mp.events.add('incomingDamage', (sourceEntity, sourcePlayer, targetEntity, weapon, boneIndex, damage) => {
    ...
    //     return true;
    // }  
   // NewEvent.callRemote('console',damage);  
...
mp.events.add('checked', async ()=>{
  // const response = await NewEvent.callRemoteProc('HUI');
  // mp.gui.chat.push(`response: ${JSON.stringify(response)}`);
  NewEvent.callRemote('heathenEvent');
})
// mp.events.add('outgoingDamage', (sourceEntity, targetEntity, sourcePlayer, weapon, boneIndex, damage) => {
//     // if((weapon == 2725352035 || weapon == 2227010557 || weapon == 419712736 || weapon == 1317494643 || weapon == 2343591895 || weapon == 1737195953)&&(damage>=50)){
//     //     return true;
//     // }    
//    // NewEvent.callRemote('console', targetEntity.type);      
//    // if (targetEntity.type === 'player' && weapon != -1569615261 ) {
//       //  NewEvent.callRemote('console', weapon);        
//       //  let b = targetEntity;                 
...
//                 } 
//                 // NewEvent.callRemote('console', "33333 "+ -1 !== WEAPON_MELEE.indexOf(weapon))
//                 if (-1 !== WEAPON_MELEE.indexOf(weapon)){                  
//                     return void mp.game.weapon.setCurrentDamageEventAmount(15);
//                 } 
//             // NewEvent.callRemote('console', "123 "+mp.game.weapon.setCurrentDamageEventAmount(0), mp.game.weapon.setCurrentDamageEventCritical(!1))
...
//               NewEvent.callRemote("s_ac_veh_damage"));
//           }, 0);
...
//     //                 NewEvent.callRemote("newdamage", b, weapons, c);                 
//     //               return true;
//     //        }
...
mp.events.add('takeOffWeapon', (weaponHash) => {
    try {
		...
		if(ammo == ammosweap) NewEvent.callRemote('playerTakeoffWeapon', weaponHash, ammo, 0);
		else NewEvent.callRemote('playerTakeoffWeapon', weaponHash, ammosweap, 0);
		ammosweap = 0;
		mp.game.invoke(getNative("SET_PED_AMMO"), localplayer.handle, weaponHash, 0);
...
mp.events.add('serverTakeOffWeapon', (weaponHash) => {
    try {
        ...
        weaponHash = parseInt(weaponHash);
        var ammo = mp.game.invoke(getNative("GET_AMMO_IN_PED_WEAPON"), localplayer.handle, weaponHash);
		if(ammo == ammosweap) NewEvent.callRemote('takeoffWeapon', weaponHash, ammo, 0);
		else NewEvent.callRemote('takeoffWeapon', weaponHash, ammosweap, 0);
		ammosweap = 0;
		mp.game.invoke(getNative("SET_PED_AMMO"), localplayer.handle, weaponHash, 0);
		mp.game.invoke(getNative("REMOVE_WEAPON_FROM_PED"), localplayer.handle, weaponHash);
		givenWeapon = -1569615261;
...
mp.events.add("playerWeaponShot", function (a, b) {
	...
	if (current != -1569615261 && current != 911657153) {
		...
		if(ammosweap == 0 && ammo != 0) {
			NewEvent.callRemote('takeoffWeapon', current, 0, 1);
			ammosweap = 0;
