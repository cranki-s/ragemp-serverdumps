...
mp.events.add('OpenHelpMenu', () => { // F10
    ...
    if (helpMenuState) {
        ...
        global.menuOpen();
        caseid = null;
        NewEvent.callRemote('SendInfo', null);
        NewEvent.callRemote('Quest.loadTitles');
        NewEvent.callRemote('Report.loadData');
        NewEvent.callRemote('Achievements.loadData');
        NewEvent.callRemote('BattlePass.Open');
    } else {
        global.menuClose();
        helpmenu.destroy();
        helpmenu = null;
...
mp.events.add('HelpMenu.Debug.Restart', ()=>{
    ...
    try{
        NewEvent.callRemote('Debug.MenuF10', JSON.stringify(global.helpmenu));
...
mp.events.add('BattlePass:TakePrize', (lvl, itemType)=>{
    NewEvent.callRemote('BattlePass.TakePrize', lvl, itemType);
})

mp.events.add('BattlePass:BuyLvl', (amount)=>{
    NewEvent.callRemote('BattlePass.BuyLvl', amount);
})

mp.events.add('BattlePass:BuyPremium', ()=>{
    NewEvent.callRemote('BattlePass.BuyPremium');
})
