...
let g_voiceMgr =
    {
        ...
        add: function (player, notify) {
            if (this.listeners.indexOf(player) === -1) {
				if (notify) NewEvent.callRemote("add_voice_listener", player);
        ...
        remove: function (player, notify) {
            ...
            if (idx !== -1) {
				if (notify) NewEvent.callRemote("remove_voice_listener", player);
...
mp.events.add('voice.phoneCall', (target) => {
    if (!PHONE.target) {
        PHONE.target = target;
        PHONE.status = true;
        NewEvent.callRemote("add_voice_listener", target);
...
mp.events.add("voice.phoneStop", () => {
    if (PHONE.target) {
        if (mp.players.exists(PHONE.target)) {
            ...
            if (dist > MaxRange) NewEvent.callRemote("remove_voice_listener", PHONE.target);
            else g_voiceMgr.add(PHONE.target, false);
        } else NewEvent.callRemote("remove_voice_listener", PHONE.target);
        PHONE.target = null;
        PHONE.status = false;
    }
});
