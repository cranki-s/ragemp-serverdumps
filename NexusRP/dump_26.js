...
mp.keys.bind(70, !1, function() {
    ...
    if (!mp.gui.cursor.visible && null == mp.players.local.vehicle) {
        ...
        if(e.testdrive!=null){
            Nexus.callRemote('TestDrive.Start', e.model>>0, e.col1, e.col2);
            return;
       }
            ...
            if (entity !== undefined) {
                if (entity.getPedInSeat(-1) == mp.players.local.handle) {
                    ...
                    if (raznica >= 0.01) {
                        ...
                        lastdirt = newdirt;
                        Nexus.callRemote("VehStream_SetVehicleDirt", entity, newdirt);
                    }
                }
            }
        }
...
mp.events.add("entityStreamIn", (entity) => {
    try {
        ...
        if (entity && mp.vehicles.exists(entity))
        {
            ...
            else Nexus.callRemote("VehStream_RequestFixStreamIn", entity);
...
mp.keys.bind(67, !0, function() {
    if (isAttachToTrunkVeh && mp.vehicles.exists(attachToTrunkVeh)) {
        ...
        Nexus.callRemote("server_vehicle_backFromTrunk", attachToTrunkVeh),mp.events.call('Hud.InfoButtons.Remove', JSON.stringify(['C'])); 
    }
}),
