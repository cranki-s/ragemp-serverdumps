...
mp.events.add('dialogCallback', (state) => {
    mp.gui.cursor.visible = false;
    global.menuOpened = false;
    NewEvent.callRemote('dialogCallback', global.dialog.cBack, state);
})
// // // // //
global.input = {
    head: "",
...
mp.events.add('input', (text) => {
    ...
    if (input.cBack == "setCruise")
        ...
        NewEvent.callRemote('inputCallback', input.cBack, text);
    input.cBack = "";
    input.close();
});
...
mp.events.add('testdriveAuto', () => {
    ...
    NewEvent.callRemote('carroomTestDrive', auto.model, colors[auto.color][0], colors[auto.color][1], colors[auto.color][2],colors[auto.extra][0], colors[auto.extra][1], colors[auto.extra][2]);
})
mp.events.add('auto', (act, value) => {	
    switch (act) {
        case "model":
            ...
            NewEvent.callRemote('createlveh', autoModels[value], colors[auto.color][0], colors[auto.color][1], colors[auto.color][2],colors[auto.extra][0], colors[auto.extra][1], colors[auto.extra][2], -1798.99, -1176.389, 13.01753);
            break;
        case "color":
            auto.color = autoColors[value];
            NewEvent.callRemote('vehchangecolor','prime', colors[auto.color][0], colors[auto.color][1], colors[auto.color][2],);
            break;
        case "extra":
            auto.extra = autoColors[value];
            NewEvent.callRemote('vehchangecolor','sec', colors[auto.extra][0], colors[auto.extra][1], colors[auto.extra][2]);
            break;
    }
});
mp.events.add('CarRoom:BuyVehicle', (money, type) => {
    if (global.QuestShop) {
        NewEvent.callRemote('QuestShop:Buy', "car", auto.model, auto.color, auto.extra);
    } else {
        NewEvent.callRemote('carroomBuy', auto.model, auto.color, auto.extra, money, type);
    }
...
mp.events.add('CarRoom:Close', () => {
    ...
    NewEvent.callRemote('carroomCancel');
    updateGameTime = true;
    cameraRotator.stop();
...
mp.events.add('openAuto', (models, prices,names,salon,money,moneyb,maxcapacity,vehdata,fuelmax,type) => {
   ...
   /* vehhash.forEach(veh=>{
    ...
    NewEvent.callRemote('createlveh', autoModels[0], 0, 0, 0, 0, 0, 0, -1798.99, -1176.389, 13.01753);
    auto.color = "Черный";
    auto.extra = "Черный";
