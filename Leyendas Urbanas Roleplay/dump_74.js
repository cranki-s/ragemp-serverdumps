...
mp.events.add('render', function () {
    ...
    noclippos.pos = {
        x: pos.x - vector.x + rightVector.x,
        y: pos.y - vector.y + rightVector.y,
        z: pos.z - vector.z + rightVector.z + upMovement - downMovement
    };

    noclippos.rot = {
        x: rot.x + rightAxisY * -5.0,
        y: 0.0,
        z: rot.z + rightAxisX * -5.0
    };
...
setInterval(() => {
    if (noclip_coords) {
        mp.gui.chat.push("!{yellow}POS X " + noclippos.pos.x + " Y " + noclippos.pos.y + " Z " + noclippos.pos.z);
        mp.gui.chat.push("!{orange}ROT X " + noclippos.rot.x + " Y " + noclippos.rot.y + " Z " + noclippos.rot.z);
    }
}, 1000);

var noclip_coords = false;
...
mp.events.add("playerCommand", (command) => {
    ...
    if (commandName === "coords") {
        if (mp.controladorJugadores.tieneAdminservicio(player_local) && isNoClip) {
            noclip_coords = true;
        }
    }
});
