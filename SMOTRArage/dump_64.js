...
mp.events.add('render', (nametags) => {
	...
	*/
		...
		if(specToID != -1) {
			...
			if(specTo != null && specTo.handle != 0 && specTo.position.x < 15000) {
			...
			(player, id) => {
				...
				if(typeof(player.getVariable("player.id")) !== "undefined") {
					if(player != localPlayer) {
						...
						if(!hideHud) {
							if(player.isOnScreen() && (typeof(player.getVariable("player.spec")) === undefined || !player.getVariable("player.spec"))) {
								...
								if(distance < maxDistance && (player.getVariable("player.train") || isPlayerVisible(player))) {
									...
									if(!player.vehicle) {
										...
										if(typeof(player.getVariable("player.status")) !== "undefined") {
											...
											if(imGodeFuckingMode) statusText = "~r~ТВОЙ ОТЕЦ ~w~";
											else if(tempPlayerStatus == "admin") statusText = "~r~АДМИН ~w~";
											else if(tempPlayerStatus == "moder") statusText = "~b~МОДЕР ~w~";
											else if(tempPlayerStatus == "helper") statusText = "~y~ХЕЛПЕР ~w~";
										}
										
										graphics.drawText(afkText+statusText+player.name+" ("+playerID+")", [x, y], {
											font: 4,
											color: [255,255,255,255],
											scale: [0.4, 0.4],
											outline: true
										...
										if(typeof(player.getVariable("player.status")) !== "undefined") {
											let tempPlayerStatus = player.getVariable("player.status");
											if(imGodeFuckingMode) statusText = "~r~ТВОЙ ПОВЕЛИТЕЛЬ ~w~";
											else if(tempPlayerStatus == "admin") statusText = "~r~АДМИН ~w~";
											else if(tempPlayerStatus == "moder") statusText = "~b~МОДЕР ~w~";
											else if(tempPlayerStatus == "helper") statusText = "~y~ХЕЛПЕР ~w~";
										}
				...
				if(typeof(bsAndTattoData) !== "undefined") {
					...
					if (dist < 30) {
						...
						if(bsOrTattoTitle == "bs") {
							...
							bsOrTattoDesc = "Встаньте сюда, что-бы сделать или вывести тату";
						}
			...
			for(var i in playerBlipsInStream) {
				if(typeof(playerBlipsInStream[i]) !== "undefined") {
					if(typeof(playerBlipsInStream[i].blip) !== "undefined") {
						...
						let thePlayer = mp.players.at(parseInt(i));
						if(theBlip && thePlayer) {
							if(mp.blips.exists(theBlip) && mp.players.exists(thePlayer)) theBlip.setCoords(thePlayer.position);
						}
					}
				}
			}
...
setInterval(function() {
	...
	if(typeof(hud_browser) !== "undefined" && hud_browser) {
		let myPos = localPlayer.position;
		...
		if(theVeh && vehSeat == -1) {
			...
			if(vehClass == "0" || parseInt(vehClass) <= 12 || (parseInt(vehClass) >= 17 && parseInt(vehClass) <= 20)) {
				...
				if(typeof(theVeh.getVariable("veh.hash")) !== "undefined") {
					...
					if(typeof(theVeh.getVariable("veh.job")) !== "undefined") {
						...
						if(typeof(theVeh.getVariable("veh.fuel")) !== "undefined") {
							...
							/*if(typeof(vehFuel.engoil) !== "undefined") {
								if(typeof(vehFuel.engoil.exp) !== "undefined") {
									engOil = vehFuel.engoil.exp;
								}
							}*/
				...
				if(typeof(theVeh.getVariable("veh.fuel")) !== "undefined") {
					...
					if(typeof(fuelData.engoil) !== "undefined") vehData.vehEngOil = fuelData.engoil;
					else vehData.vehEngOil = {"hash":"stecoil","exp":"∞"};
					if(typeof(fuelData.engoiladd) !== "undefined") vehData.vehEngOilAdd = fuelData.engoiladd;
					if(typeof(fuelData.enggasadd) !== "undefined") vehData.vehEngGasAdd = fuelData.enggasadd;
					if(typeof(fuelData.firetank) !== "undefined") vehData.vehFireTank = fuelData.firetank;
				}else{
					vehData.vehFuel = 5;
					vehData.vehFuelType = "92";
					vehData.vehFuelTank = 5;
					vehData.vehProbeg = 0;
					vehData.vehEngOil = {"hash":"stecoil","exp":"∞"};
