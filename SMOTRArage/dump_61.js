...
function checkFreeNumber(vehNumTypeTemp, numForCheck) {
	if(hud_browser && numForCheck && numForCheck.length == 8) {
		numForCheck = numForCheck.toString();
		mp.events.callRemote('checkFreeNumber', vehNumTypeTemp, numForCheck);
	}
}
...
function numberBuy(vehNumTypeTemp, numForCheck, cost) {
	if(hud_browser && numForCheck && numForCheck.length == 8) {
		cost = parseInt(cost);
		...
		if(localPlayer.getVariable('player.id') && localPlayer.getVariable('player.money') && theVeh.getVariable('veh.id') && theVeh.getVariable('veh.num') && theVeh.getVariable('veh.own') && theVeh.getVariable('veh.params')) {
			if(mp.players.atRemoteId(parseInt(theVeh.getVariable('veh.own')))) {
				...
				if(vehOwn.remoteId.toString() == localPlayer.remoteId.toString()) {
					...
					if(vehClass.toString() == "15") return hud_browser.execute("numBuyBlankError('На вертолёте нельзя');");
					...
					if(vehNumTypeTemp != "ruVeh" && vehNumTypeTemp != "ruMoto") return hud_browser.execute("numBuyBlankError('На данный транспорт нельзя');");
					
					numForCheck = numForCheck.toString();
					mp.events.callRemote('numberBuy', vehNumTypeTemp, numForCheck, cost);
				}
			}
		}
	}
		...
		if(typeof(checkpoint.mreoData) !== "undefined") {
			...
			if(localPlayer.vehicle && hud_browser) {
				...
				if(localPlayer.getVariable('player.id') && localPlayer.getVariable('player.money') && theVeh.getVariable('veh.id') && theVeh.getVariable('veh.num') && theVeh.getVariable('veh.own')) {
					...
					if(mp.players.atRemoteId(parseInt(theVeh.getVariable('veh.own')))) {
						let vehOwn = mp.players.atRemoteId(parseInt(theVeh.getVariable('veh.own')));
						if(vehOwn.remoteId.toString() != localPlayer.remoteId.toString()) return notyAPI.error("Регистрационные действия разрешены только с личным транспортом.", 3000, true);
					}else{
						return notyAPI.error("Регистрационные действия разрешены только с личным транспортом.", 3000, true);
					}
					
					//if(theVeh.getVariable('veh.num') == "theMoto") return notyAPI.error("Регистрационные действия для мото-техники недоступны.", 3000, true);
					if(vehClass.toString() == "15") return notyAPI.error("Регистрационные действия для вертолётов недоступны.", 3000, true);
					
					if(typeof(theVeh.getVariable('veh.params')) !== "undefined") {
						let vehParams = JSON.parse(theVeh.getVariable("veh.params"));
						if(typeof(vehParams.rent) !== "undefined") return notyAPI.error("Рег. действия на арендованном транспорте запрещены.", 3000, true);
					}
					
					let vehNum = JSON.parse(theVeh.getVariable('veh.num'));
					if(typeof(vehNum.type) === "undefined") return notyAPI.error("Регистрационные действия на данном транспорте - недоступны.", 3000, true);
					
					allowBinds = [];
					hud_browser.execute('toggleNumBuyPanel(\'1\', \'{"vid":"'+theVeh.getVariable('veh.id')+'","numType":"'+vehNum.type+'","number":"'+vehNum.num+'","money":"'+localPlayer.getVariable('player.money')+'"}\');');
