...
mp.keys.bind(0x26, true, function() { // стрелка вверх, телефон
	...
	if(afTogglePhone) return false;
	afTogglePhone = true;
	setTimeout(function() { afTogglePhone = false }, 500);
	if(hud_browser) {
		if(!mobilePhone) {
			mobilePhone = true;
...
mp.events.add("toggleAirMode", () => {
	if(typeof(localPlayer.getVariable("player.id")) !== "undefined") {
		afAirmode = true;
		setTimeout(function() { afAirmode = false; }, 30000);
...
mp.events.add("togglePassive", () => {
	if(typeof(localPlayer.getVariable("player.id")) !== "undefined" && typeof(localPlayer.getVariable("player.passive")) !== "undefined") {
		...
		if(afPassive) return hud_browser.execute('togglePhone(\'{"name":"passive","toggled":'+localPlayer.getVariable("player.passive")+',"error":"Подождите, раз в 30 сек."}\');');
		...
		afPassive = true;
		setTimeout(function() { afPassive = false; }, 30000);
...
mp.events.add("sendReport", (reportObj, reportText) => {
	if(hud_browser) {
		if(typeof(localPlayer.getVariable("player.id")) !== "undefined") {
			if(afReport) return hud_browser.execute('togglePhone(\'{"name":"report","error":"Доступно раз в минуту"}\');');
			afReport = true;
			setTimeout(function() { afReport = false }, 60000);
