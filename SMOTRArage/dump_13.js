				...
				if(localPlayer.getVariable('player.id')) {
					...
					if(tuningVehicle && hud_browser) {
						...
						if(typeof(tuningVehicle.getVariable('veh.params')) !== "undefined") {
							...
							if(typeof(vehParams.rent) !== "undefined") return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Арендованый транспорт нельзя тюнинговать</span>");
						}
						...
						if(typeof(tuningVehicle.getVariable('veh.own')) !== "undefined") {
							if(mp.players.atRemoteId(parseInt(tuningVehicle.getVariable('veh.own')))) {
								...
								if(vehOwn.remoteId.toString() != localPlayer.remoteId.toString()) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Воспользоваться тюнингом можно только на личном ТС</span>");
							}else{
								return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Воспользоваться тюнингом можно только на личном ТС</span>");
							}
						}else{
							return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Воспользоваться тюнингом можно только на личном ТС</span>");
						}
						...
						if(tuningVehicle.getClass().toString() == "15") {
							if(tunData[4] != "heli") return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Тут не обслуживают вертолёты, найдите другую станцию..</span>");
						}else{
							if(tunData[4] == "heli") return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Тут обслуживают только вертолёты, найдите другую станцию..</span>");
						}
						
						if(tuningVehicle.getNumberOfPassengers() > 1) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Высадите пассажиров, что бы попасть в тюнинг.</span>");
						...
						chatAPI.sysPush("<span style=\"color:#FF6146\"> * Чтобы воспользоваться тюнингом, необходимо использовать транспорт</span>");
					}
					return false;
				}
			}
				...
				if(!localPlayer.vehicle && hud_browser) {
					...
					if(oldTuning !== undefined && Object.keys(oldTuning).length > 0) {
						...
						if(garTunData[3] == "paints") {
							...
							if(hud_browser) {
								if(typeof(tuningVehicle.getVariable("veh.num")) !== "undefined") {
									let vehNum = tuningVehicle.getVariable("veh.num");
									if(vehNum == "theMoto") return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Я не буду заниматься мото-техникой..</span>");
								}
							...
							if(hud_browser) {
								...
								let tuningData = {"kits":0,"spoilers":0,"fbumps":0,"bbumps":0,"skirts":0,"hoods":0,"fenders":0,"roofs":0};
