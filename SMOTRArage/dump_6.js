	...
	if(entity && entity.type == 'vehicle' && value && entity.handle != 0) {
		...
		if(typeof(value.type) !== "undefined" && typeof(value.num) !== "undefined") {
			if(value.type == "ruVeh") {
				...
				entity.numberPlateType = 4;
				entity.setNumberPlateTextIndex(4);
			}else if(value.type == "ruHeli") {
				if(parseInt(entity.isExtraTurnedOn(1)) != 1) entity.setExtra(1, 0);
				if(parseInt(entity.isExtraTurnedOn(2)) != 1) entity.setExtra(2, 0);
				let num = value.num.toString();
...
mp.events.addDataHandler('veh.anchor', function (entity, value, oldValue) {
	if(entity && entity.type == 'vehicle' && entity.handle != 0) {
		if(typeof(value) !== "undefined") entity.freezePosition(value);
	}
});
...
function reSyncVehicle(entity) {
	if(entity && entity.handle != 0) {
		...
		*/
		...
		// Номера
		if(entity.getVariable("veh.num")) {
			let numberData = JSON.parse(entity.getVariable("veh.num"));
			if(typeof(numberData.type) !== "undefined" && typeof(numberData.num) !== "undefined") {
				if(numberData.type == "ruVeh") {
					if(parseInt(entity.isExtraTurnedOn(5)) != 1) entity.setExtra(5, 0);
					if(parseInt(entity.isExtraTurnedOn(6)) != 1) entity.setExtra(6, 0);
					let num = numberData.num.toString();
					num = num.split("|").join("");
					num = num.split(" ").join("");
					entity.setNumberPlateText(num);
					entity.numberPlateType = 4;
					entity.setNumberPlateTextIndex(4);
				}else if(numberData.type == "ruMoto") {
					if(parseInt(entity.isExtraTurnedOn(5)) != 1) entity.setExtra(5, 0);
					if(parseInt(entity.isExtraTurnedOn(6)) != 1) entity.setExtra(6, 0);
					let num = numberData.num.toString();
					num = num.split("|").join("");
					...
					entity.numberPlateType = 4;
					entity.setNumberPlateTextIndex(4);
				}else if(numberData.type == "ruHeli") {
					if(parseInt(entity.isExtraTurnedOn(1)) != 1) entity.setExtra(1, 0);
					if(parseInt(entity.isExtraTurnedOn(2)) != 1) entity.setExtra(2, 0);
					let num = numberData.num.toString();
					num = str_replace_all(num, "-", "Z");
		...
		if(typeof(entity.getVariable("veh.anchor")) !== "undefined") entity.freezePosition(entity.getVariable("veh.anchor")); // Якорь
...
mp.events.addDataHandler('veh.frac', function (entity, value, oldValue) {
	if(entity && entity.type == 'vehicle' && entity.handle != 0) {
		if(value) {
		...
		switch(entity.type) {
			...
			case 'vehicle':
				...
				*/
				...
				if(typeof(entity.getVariable('veh.frac')) !== 'undefined') {
					entity.setSirenSound(true);
				}
				...
				if(typeof(CryptoJS) !== "undefined" && typeof(vehStats) !== "undefined" && typeof(krKey) !== "undefined") {
					...
					if(typeof(entity.getVariable("veh.type")) !== "undefined" && typeof(entity.getVariable("veh.own")) !== "undefined" && typeof(entity.getVariable("veh.warp")) !== "undefined") {
						if((entity.getVariable("veh.type") == "rentVeh" || entity.getVariable("veh.type") == "frac") && entity.getVariable("veh.warp")) {
							if(!localPlayer.vehicle) {
