...
function unLockStopWork(isNaloged) {
	if(typeof(isNaloged) !== "undefined") {
		...
		if(isNaloged) notyAPI.success("С зарплаты удержан подоходный налог в 13%, Ваша зарплата:"+isNaloged.toString().replace(/(\d{1,3})(?=((\d{3})*)$)/g, " $1")+" руб.", 3000, true);
	}
}
	...
	if(typeof(shape.data) == 'undefined' && typeof(shape.id) != "undefined") {
		if(typeof(shape.getVariable('col.type')) != "undefined") {
			...
			if(colType == 'job_render') {
				...
				let jobCheck = mp.checkpoints.new(40, new mp.Vector3(parseFloat(jobData[0]), parseFloat(jobData[1]), parseFloat(jobData[2])), 1.5,
				{
					color: [255, 255, 255, 0],
					visible: true,
			...
			if(localPlayer.getVariable("player.job") && !localPlayer.vehicle && hud_browser) {
				...
				if(myJobData.name) {
					if(myJobData.name == jobData[3].toString()) {
						notyAPI.success("Вы уже работаете на этой работе.", 3000, true);
						return notyAPI.info("Используйте рабочий планшет для управления сменами [ <b>F5</b> ].", 3000, false);
					}else{
						notyAPI.error("Что бы устроится сюда, необходимо быть безработным.", 3000, true);
						return notyAPI.info("Увольтесь с текущей работы, через планшет [ <b>F5</b> ].", 3000, false);
					}
				}else{
					...
					}else if(jobData[3].toString() == "bus") {
						if(hud_browser) {
							hud_browser.execute('toggleJobsPanel(\'{"name":"jobStartBus"}\')');
							mp.gui.cursor.visible = true;
						}
...
function checkVehInWorkOnJob(vehicle, seat) {
	if(vehicle && localPlayer.getVariable("player.job")) {
		if(parkingVeh) {
			if(curCourierTask) {
				if(typeof(curCourierTask.courier) !== "undefined") {
					if(typeof(vehicle.getVariable("veh.id")) !== "undefined") {
						if(vehicle.getVariable("veh.id").toString() == curCourierTask.courier) {
							parkingVeh = vehicle;
							if(jobVehBackTimer) {
								clearTimeout(jobVehBackTimer);
								mp.game.ui.messages.showMidsized("~s~Вы вернулись в ~g~курьерский транспорт", "~s~Теперь всё в полном порядке ^_^");
							}
						}
					}
				}
			...
			if(curTruckTask) {
				if(typeof(curTruckTask.curPoint) !== "undefined") {
								...
								if(typeof(trailersPool) !== "undefined") {
									if(typeof(trailersPool[vehicle.handle.toString()]) !== "undefined") {
										if(typeof(trailersPool[vehicle.handle.toString()].trailer) !== "undefined") {
											...
											if(mp.vehicles.exists(tempTrailer) && mp.vehicles.exists(vehicle)) parkingVeh = tempTrailer;
										}
									}
								}
							}else{
			...
			if(curBusTask) {
				if(typeof(curBusTask.curPoint) !== "undefined") {
					if(typeof(vehicle.getVariable("veh.job")) !== "undefined") {
						let vehJob = mp.players.atRemoteId(parseInt(vehicle.getVariable('veh.job')));
						if(vehJob.remoteId.toString() == localPlayer.remoteId.toString()) parkingVeh = vehicle;
					}
				}
			}
		}
		...
		if(typeof(myJobData.name) !== 'undefined') {
			if(curCourierTask) {
				if(typeof(curCourierTask.courier) !== "undefined") {
					if(typeof(vehicle.getVariable("veh.id")) !== "undefined") {
						if(vehicle.getVariable("veh.id").toString() == curCourierTask.courier) {
							jobVehBackTimer = setTimeout(courierStartStop, 30000);
							notyAPI.error("Вы покинули курьерский транспорт, у Вас есть 30 секунд что бы вернуться в него.", 3000, true);
							mp.game.ui.messages.showMidsized("~s~Вы покинули ~r~курьерский транспорт", "~s~У Вас есть 30 секунд что бы вернуться в него.\nИначе - смена будет закрыта автоматически.");
						}
					}
				}
			}
				...
				if(mp.players.atRemoteId(parseInt(vehicle.getVariable('veh.job')))) {
					...
					if(vehJob.remoteId.toString() == localPlayer.remoteId.toString()) {
						if(typeof(myJobData.work) !== "undefined") {
							if(myJobData.work == 1) {
								if(myJobData.name != "air") {
									notyAPI.error("Вы покинули рабочий транспорт, у Вас есть 5 минут что бы вернуться в него.", 3000, true);
									...
									else if(myJobData.name == "bus") jobVehBackTimer = setTimeout(truckStartStop, 300000);
								}else{
									if(curAirTask) {
										notyAPI.error("Вы покинули рабочий транспорт, задача отменена.", 3000, true);
