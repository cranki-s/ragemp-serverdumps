...
function fishingStart() {
    if(fishingMode && hud_browser) {
		if(typeof(localPlayer.getVariable("player.inv")) === "undefined") return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Инвентарь не инициализирован, рыбалка недоступна..</span>");
		let myInv = localPlayer.getVariable("player.inv");
		if(typeof(myInv["instrument"]) !== "undefined") {
			if(parseInt(myInv["instrument"].health) <= 0) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Рыболовная снасть слишком изношена..</span>");
		}
		
		if(typeof(localPlayer.getVariable("active.deal")) !== "undefined") {
			if(localPlayer.getVariable("active.deal")) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * У Вас есть активная сделка, рыбалка недоступна..</span>");
		}
		...
		if(breadCount == 0 && insectsCount == 0 && wormsCount == 0 && bloodwormCount == 0 && minfishCount == 0) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * У Вас нет наживок, купите их в магазине 24/7..</span>");
...
function baitFishSelected(baitData) {
	if(hud_browser && baitData) {
		...
		if(typeof(localPlayer.getVariable("player.inv")) === "undefined") {
			restoreBinds();
			return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Инвентарь не инициализирован, рыбалка недоступна..</span>");
		}
		
		if(typeof(localPlayer.getVariable("active.deal")) !== "undefined") {
			if(localPlayer.getVariable("active.deal")) {
				restoreBinds();
				return chatAPI.sysPush("<span style=\"color:#FF6146\"> * У Вас есть активная сделка, рыбалка недоступна..</span>");
			}
		}
		...
		if(baitData.bait == "bread" && breadCount < 1) {
			restoreBinds();
			return chatAPI.sysPush("<span style=\"color:#FF6146\"> * У Вас нет хлебного мякиша, выберите другую наживку..</span>");
		}else if(baitData.bait == "insects" && insectsCount < 1) {
			restoreBinds();
			return chatAPI.sysPush("<span style=\"color:#FF6146\"> * У Вас нет личинок насекомых, выберите другую наживку..</span>");
		}else if(baitData.bait == "worms" && wormsCount < 1) {
			restoreBinds();
			return chatAPI.sysPush("<span style=\"color:#FF6146\"> * У Вас нет дождевых червей, выберите другую наживку..</span>");
		}else if(baitData.bait == "bloodworm" && bloodwormCount < 1) {
			restoreBinds();
			return chatAPI.sysPush("<span style=\"color:#FF6146\"> * У Вас нет личинок мотыля, выберите другую наживку..</span>");
		}else if(baitData.bait == "minifish" && minfishCount < 1) {
			restoreBinds();
			return chatAPI.sysPush("<span style=\"color:#FF6146\"> * У Вас нет мальков, выберите другую наживку..</span>");
		}
		
		if(dealerPanel) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * У Вас есть активная сделка с барыгой..</span>");
