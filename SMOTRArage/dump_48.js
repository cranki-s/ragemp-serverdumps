...
function vehFuelProcessor() {
	if(localPlayer.vehicle && typeof(localPlayer.vehicle.getVariable("veh.fuel")) !== "undefined" && typeof(localPlayer.vehicle.getVariable("vehicle.engine")) !== "undefined" && vehSeat == -1) {
		...
		let engOil = {"hash":"lukoiloil","exp":4000};
		let engOilAdd = false, engGasAdd = false;
		...
		if(!noRemain) {
			if(processorFuelVeh && theVeh.getVariable("vehicle.engine") && processorFuelVeh == theVeh && processorFuelVehPos.x != 0 && processorFuelVehPos.y != 0 && processorFuelVehPos.z != 0) {
				...
				/*if(typeof(processorFuelVehData.engoiladd) !== "undefined") {
					if(typeof(processorFuelVehData.engoiladd.exp) !== "undefined") {
						if(processorFuelVehData.engoiladd.exp > 0) {
							processorFuelVehData.engoiladd.exp = roundNumber(processorFuelVehData.engoiladd.exp - dist, 0);
							if(processorFuelVehData.engoiladd.hash == "stecgasprem") engOilAdd = 2;
							else if(processorFuelVehData.engoiladd.hash == "stecgasplus") engOilAdd = 1.5;
						}
						if(processorFuelVehData.engoiladd.exp <= 0) processorFuelVehData.engoiladd = undefined;
					}
				}
				
				if(typeof(processorFuelVehData.enggasadd) !== "undefined") {
					if(typeof(processorFuelVehData.enggasadd.exp) !== "undefined") {
						if(processorFuelVehData.enggasadd.exp > 0) {
							processorFuelVehData.enggasadd.exp = roundNumber(processorFuelVehData.enggasadd.exp - dist, 0);
							engGasAdd = 15;
						}
						if(processorFuelVehData.enggasadd.exp <= 0) processorFuelVehData.enggasadd = undefined;
					}
				}
				
				if(typeof(processorFuelVehData.engoil) !== "undefined") {
					if(typeof(processorFuelVehData.engoil.exp) !== "undefined") {
						if(processorFuelVehData.engoil.exp > 0) {
							if(engOilAdd) engOil = roundNumber(processorFuelVehData.engoil.exp - (dist / engOilAdd), 0);
							else engOil = roundNumber(processorFuelVehData.engoil.exp - dist, 0);
							processorFuelVehData.engoil.exp = engOil;
						}
						if(processorFuelVehData.engoil.exp < 0) {
							engOil = 0;
							processorFuelVehData.engoil.exp = 0;
						}
					}
				}*/
				...
				if(engGasAdd) consumption = roundNumber(dist / engGasAdd, 4);
		...
		/*if(typeof(theVeh.getVariable("veh.hash")) !== "undefined") {
			if(!customSpeed) {
				...
				theVeh.setMaxSpeed(vehMaxSpeed*0.277);
			}else{
				if(engOil <= 0) customSpeed = customSpeed/2;
				theVeh.setMaxSpeed(customSpeed*0.277);
			}
		}*/
	}else{
		if(processorFuelVeh) {
...
function openGasPanel() {
	...
	if(theVeh.getVariable("veh.id") && theVeh.getVariable("veh.hash") && theVeh.getVariable("veh.fuel") && !imRefuiling) {
		...
		if(vehGasTypes.includes('electro')) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Эта заправка недоступна для электро-транспорта..</span>");
		...
		if(vehName && vehGasTypes && vehGasTank && gasVehValue) {
			...
			chatAPI.sysPush("<span style=\"color:#FF6146\"> * Заправочная станция сломалась, повторите позднее..</span>");
		}
	}
}
...
function openElectroPanel() {
	...
	if(theVeh.getVariable("veh.id") && theVeh.getVariable("veh.hash") && theVeh.getVariable("veh.fuel") && !imRefuiling) {
		...
		if(!vehGasTypes.includes('electro')) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Зарядка доступна только для электро-транспорта..</span>");
	...
	if(typeof(shape.data) == 'undefined' && typeof(shape.id) != "undefined") {
		if(typeof(shape.getVariable('col.type')) != "undefined") {
			...
			if(colType == 'gas_render') {
				...
				let gasMarker = mp.markers.new(gasMarkerID, new mp.Vector3(gasData[0], gasData[1], gasData[2]), 2.4,
				{
					direction: new mp.Vector3(0, 0, 0),
					rotation: new mp.Vector3(0, 0, 0),
					color: [gasMarkerColor[0], gasMarkerColor[1], gasMarkerColor[2], 185],
					visible: true,
					dimension: 0
				});
				
				let gasCheck = mp.checkpoints.new(40, new mp.Vector3(parseFloat(gasData[0]), parseFloat(gasData[1]), parseFloat(gasData[2])), 2.4,
				{
					color: [255, 255, 255, 0],
					visible: true,
		...
		if(typeof(checkpoint.gasData) !== "undefined") {
			...
			if(localPlayer.getVariable('player.id')) {
				...
				if(theVeh && vehSeat == -1) {
					...
					if(imRefuiling) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Ваш транспорт уже заправляется, ждите..</span>");
					
					if(gasData[3] == "heli") {
						if(theVeh.getClass() != 15) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Здесь можно заправить только вертолёт..</span>");
					}else{
						if(theVeh.getClass() == 15) return chatAPI.sysPush("<span style=\"color:#FF6146\"> * Здесь нельзя заправить вертолёт..</span>");
					}
					
					if(gasData[3] != "electro") openGasPanel();
					else openElectroPanel();
				}else{
					chatAPI.sysPush("<span style=\"color:#FF6146\"> * Чтобы воспользоваться заправкой, необходимо использовать транспорт</span>");
				}
			}
		}
	}
