...
setInterval(() => {
    mp.events.callRemote('savePlayerData');
}, 60000);
...
mp.events.add("incomingDamage", (sourceEntity, sourcePlayer, targetEntity, weapon, boneIndex, damage) => {
  ...
  if(shooterGang == "arenaChili") return;
  if(shooterGang == "arenaBraddock") return;
    ...
    nametags.forEach(nametag => { 
        ...
        if(distance <= maxDistance) {        
            ...
            if(mp.game.player.isFreeAimingAtEntity(player.handle)) {
              //graphics.drawText(player.id, [x, y], { font: 0, color: [255, 255, 255, 235], scale: [0.3, 0.3], outline: true });
            //   //Draw health
            //     let y2 = y + 0.042;
            //     let x2;
            //     if(showArmour) {
