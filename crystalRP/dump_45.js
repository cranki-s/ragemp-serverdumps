...
var casinoRoulette = null;
mp.events.add('openCasinoRoulette', (data) => {  
    if (casinoOpen || casinoRoulette != null) return;
    casinoOpen = true;
    menuOpen();
	casinoRoulette = mp.browsers.new('package://cef/casino/casinobet/index.html');
    casinoRoulette.execute(`casino.show('${data}')`);
});
mp.events.add('closeCasinoRoulette', () => {
    if (!casinoOpen || casinoRoulette == null) return;
    casinoOpen = false;
    casinoRoulette.destroy();
	casinoRoulette = null;
    menuClose();
});
mp.events.add('updateCasinoTime', (data) => {
	if (casinoRoulette == null) return;
    casinoRoulette.execute(`casino.setTimeToStart('${data}')`);
});
mp.events.add('updateCasinoChips', (data) => {
	if (casinoRoulette == null) return;
    casinoRoulette.execute(`casino.setChips('${data}')`);
});
mp.events.add('start_roulette', function(){
	if (casinoRoulette == null) return;
    global.startRoullete = true;
	casinoRoulette.execute(`casino.hide()`);
});
mp.events.add('stop_roulette', function(){
	if (casinoRoulette == null) return;
    global.startRoullete = false;
    count = 0;
	casinoRoulette.execute(`casino.rest()`);
});
mp.keys.bind(0x1b, false, function() {
    if (!loggedin || chatActive || !casinoOpen || editing || new Date().getTime() - lastCheck < 1000 || global.startRoullete || casinoRoulette == null) return;
    mp.events.call("closeCasinoRoulette");
...
let podium = mp.objects.new(podiumHash, new mp.Vector3(1100.077,219.9723,-50.084865),{dimension: 10});
...
for(let tbs = 0; tbs < RouletteTablesPos.length; tbs++){
    ...
    RouletteTables[tbs].ball = mp.objects.new(87196104, new mp.Vector3( RouletteTablesPos[tbs].x, RouletteTablesPos[tbs].y, RouletteTablesPos[tbs].z),{dimension: 10});
}
}, 1000);
...
setTimeout(function () {
    ...
    CasinoPeds.forEach(ped => {
        CasinoPedsID[n] = mp.peds.new(ped.Hash, ped.Pos, ped.Angle, 10);
...
mp.events.add('render', () => {
    ...
    if(setBet && rouletteCamera != null && rouletteTable != null && startRoullete == false && !mp.gui.cursor.visible){
        ...
        {
            betObject = mp.objects.new(mp.game.joaat("vw_prop_chip_100dollar_x1"), new mp.Vector3(RouletteTablesPos[rouletteTable].x, RouletteTablesPos[rouletteTable].y, RouletteTablesPos[rouletteTable].z + 0.4),{dimension: 10});
        ...
        if(betObject != null && canDoBets)
		{
			if(mp.game.controls.isDisabledControlJustReleased(0, 25) && !mp.gui.cursor.visible) // ПКМ
				if(closestChipSpot != null) 
                    mp.events.callRemote("server_remove_roulette_bet", closestChipSpot);
			if (mp.game.controls.isDisabledControlJustReleased(0, 24) && !mp.gui.cursor.visible) // ЛКМ
				if (closestChipSpot != null && count < 3) {
                    count++;
                    mp.events.callRemote("server_make_roulette_bet", closestChipSpot);
                }
                else
                    mp.events.call('notify', 4, 9, "Вы поставили максимальное кол-во ставок", 3000);
		...
		if(spot != null)
		{
			...
			for(var i=0; i < tableChipsOffsets[spot][3].length; i++)
			{
				...
				if(key == "00" || key == "0")
				{
                    ...
                    obj = mp.objects.new(269022546, new mp.Vector3(newCardPos.x, newCardPos.y, newCardPos.z), { dimension: 10,rotation: new mp.Vector3(0, 0, RouletteTablesHeading[rouletteTable])});
				...
				{
                    let newCardPos = mp.game.object.getObjectOffsetFromCoords(RouletteTablesPos[rouletteTable].x, RouletteTablesPos[rouletteTable].y, RouletteTablesPos[rouletteTable].z, RouletteTablesHeading[rouletteTable], tableMarkersOffsets[key][0], tableMarkersOffsets[key][1], tableMarkersOffsets[key][2]);      
                    tableMarkers.push(mp.objects.new(3267450776, new mp.Vector3(newCardPos.x, newCardPos.y, newCardPos.z), {dimension: 10,rotation: new mp.Vector3(0, 0, RouletteTablesHeading[rouletteTable])}));
				}
			}
		}
	}	
