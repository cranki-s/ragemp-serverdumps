					...
					if (player.handle !== 0 && player !== mp.players.local) {
						if(myalvl >= player.getVariable('ALVL')) {
							...
							if(player.getVariable('IS_ADMIN')) {
								mp.game.graphics.drawText(_playerName + ` ID: ${player.remoteId} \n HP ${player.getHealth()}, AR ${player.getArmour()}, #${player.getVariable("UID")}`, [position.x, position.y, position.z+1.5], {
									scale: [0.3, 0.3],
									outline: true,
									color: [255, 255, 0, 255],
									font: 4
