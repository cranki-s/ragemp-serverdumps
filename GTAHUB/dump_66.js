...
mp.rpc("player:set_passive", (toggle) => {
    passive = toggle;
});
...
mp.rpc("player:put_into_vehicle", (vehicleId, position, seat) => {
    let vehicle = mp.vehicles.atRemoteId(vehicleId);
    if (vehicle) {
        putIntoVehicle.vehicle = vehicle;
        putIntoVehicle.seat = seat - 1;
        putIntoVehicle.position = JSON.parse(position);
        putIntoVehicle.lastEnter = Date.now();
    } else putIntoVehicle.vehicle = null;
});
...
mp.setInterval(() => {
    ...
    if (putIntoVehicle.vehicle) {
        let localPosition = mp.players.local.position;
        if (mp.game.system.vdist(localPosition.x, localPosition.y, localPosition.z, putIntoVehicle.position.x, putIntoVehicle.position.y, putIntoVehicle.position.z) > 50) {
            mp.console.logInfo(`moving position to ${JSON.stringify(putIntoVehicle.position)}`)
            mp.players.local.setCoords(
                putIntoVehicle.position.x,
                putIntoVehicle.position.y,
                putIntoVehicle.position.z,
                true, false, false, false
            );
        }
        if (mp.players.local.vehicle !== putIntoVehicle.vehicle && putIntoVehicle.vehicle.handle) {
            mp.players.local.taskEnterVehicle(putIntoVehicle.vehicle.handle, -1, putIntoVehicle.seat, 1.0, 16, 0);
        }
        if (mp.players.local.vehicle === putIntoVehicle.vehicle || Date.now() - putIntoVehicle.lastEnter > 2000) {
            putIntoVehicle.vehicle = null;
        }
    }
...
mp.events.add("render", () => {
    ...
    if (!canUseMelee || meleeLock > time || passive) {
