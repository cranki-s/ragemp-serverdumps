...
function rappelPlayer(warningLevel)
{
    ...
    if (!mp.game.invoke("0x4E417C547182C84D", vehicle.handle)) {
        if(warningLevel == 1) pushMessageToChat("You cannot rappel from this vehicle.");
        return;
    }

    if (vehicle.getSpeed() > maxSpeed) {
        if(warningLevel >= 1) pushMessageToChat("The vehicle is too fast for rappelling.");
        return;
    }

    if (vehicle.getPedInSeat(-1) === localPlayer.handle || vehicle.getPedInSeat(0) === localPlayer.handle) {
        if(warningLevel >= 1) pushMessageToChat("You cannot rappel from your seat.");
        return;
    }
    ...
    if (curHeight < minHeight || curHeight > maxHeight) {
        pushMessageToChat("The vehicle is too low/too high for rappelling.");
        return;
    }

    if (!vehicle.isUpright(maxAngle) || vehicle.isUpsidedown()) {
        pushMessageToChat("The vehicle needs to be stable for rappelling.");
        return;
    }
...
mp.events.add('OneSecondEvent', () => {
    if(mp.storage.data.menu.tooltips == true && UIHud != null)
    {
