...
mp.events.callRemote('noclip_pos_update', noClipCamera.position.x, noClipCamera.position.y, noClipCamera.position.z);
  if (noclip_frozen)
    mp.gui.chat.push("[Режим маппинга] Теперь вы заморожены, вы можете перемещать мебель, снова нажмите F6, чтобы переместить камеру.");
  else
    mp.gui.chat.push("[Режим маппинга] Движения включены. Мебель больше нельзя перемещать, пока вы снова не нажмете F6.");
});
...
mp.events.add("ToggleNoClip", () => {
    ...
    if (isNoClip) {
      mp.events.callRemote('NoclipInvisOn');
      startNoClip();
    } else {
      mp.events.callRemote('NoclipInvisOff');
      stopNoClip();
    }
});

function startNoClip() {
  mp.game.graphics.notify('Режим маппинга ~g~активирован');
...
function stopNoClip() {
  mp.game.graphics.notify('Режим маппинга ~r~деактивирован');
  if (noClipCamera) {
  ...
  if(!spectating)
  {
    ...
    mp.events.callRemote('OnNoClipEnd');
  }
