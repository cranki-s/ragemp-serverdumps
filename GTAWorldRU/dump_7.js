...
var onSubmitCharacter = function (firstname, lastname, jailChar) {
    if(!firstname.match(validPattern) || !lastname.match(validPattern)){
        mp.game.graphics.notify("~r~Используйте буквы латинского алфавита!");
        return;
    }
    if (cef != null) {
		cef.destroy();
...
var onSubmitInviteFaction = function (string) {
    ...
    mp.gui.chat.push("Приглашение успешно отправлено!");
...
var onSubmitCreateFaction = function (faction_name, faction_short_name) {
	...
	mp.gui.chat.push("Вы успешно создали фракцию!");
...
var onSubmitGiveMoney = function (playername, money) {
    ...
    mp.gui.chat.push("Запрос на перевод средств одобрен!");
...
class BlipHelper {
    ...
    static createBlipSpec(name, position, color, type) {
        if (blips.length != 0 && blips[name] !== undefined && blips[name] !== null){
            blips[name].setCoords(position);
            return;
        }

        var blip = mp.blips.new(type, position,
        {
            name: name,
            color: color,
...
var garbagePickedUp = "Мусора загружено";
...
var rappelVehicles = [-1660661558, 353883353, 837858166, 2774770703]; // maverick, emergency maverick, annihilator, sahpair
...
mp.events.add('rappel', function() {
    ...
    if (!mp.game.invoke("0x4E417C547182C84D", vehicle.handle)) {
        mp.gui.chat.push("Вы не можете спуститься с этого транспортного средства.");
        return;
    }

    if (vehicle.getSpeed() > rappelmaxSpeed) {
        mp.gui.chat.push("Транспортное средство слишком быстро летит для спуска.");
        return;
    }

    if (vehicle.getPedInSeat(-1) === localPlayer.handle || vehicle.getPedInSeat(0) === localPlayer.handle) {
        mp.gui.chat.push("Вы не можете спрыгнуть со своего места.");
        return;
    }

    const taskStatus = localPlayer.getScriptTaskStatus(-275944640);
    if (taskStatus === 0 || taskStatus === 1) {
        mp.gui.chat.push("Вы уже спускаетесь.");
        return;
    }

    const curHeight = vehicle.getHeightAboveGround();
    if (curHeight < rappelMinHeight || curHeight > rappelMaxHeight) {
        mp.gui.chat.push("Транспортное средство слишком низко / слишком высоко для спуска.");
        return;
    }

    if (!vehicle.isUpright(rappelmaxAngle) || vehicle.isUpsidedown()) {
        mp.gui.chat.push("Транспортное средство должно быть стабилизированным для спуска.");
        return;
    }
...
function createPartMenu(strPartName, intVariants, intStockIndex, startIndex = 0) {
    ...
    if(strPartName == "Колеса"){
        ...
        menuListItem = new UIMenuListItem("Тип " + strPartName, displayPrice, new ItemsCollection(lstOptions2), intSelectedOption);
        tmpPartMenu.AddItem(menuListItem);
    }

    // Don't say installed for turbo and xenon lights (getMod doesn't work for them)
    let excludeInstalled = false;
    if(strPartName == "Турбо" || strPartName == "Ксенон" || strPartName == "Колеса") excludeInstalled = true;

    for(let i = 0; i <= intVariants; i++) {

        
        if(i == 0) {
            if(CurrentPart == 0 && !excludeInstalled) lstOptions.push("Сток - Установлено");
            else lstOptions.push("Сток");
        }
        else {
            if(CurrentPart != i || excludeInstalled) lstOptions.push(`${i}`);
            else lstOptions.push(`${i} - Установлено`);
        }
    }
    if(strPartName == "Колеса")
    ...
    tmpPartMenu.ListChange.on(function (item, index) {
        if(item.Text == "Тип колёс"){
            ...
            let tmpPartMen = createPartMenu("Колеса", vehicleModLimit, 0, startIndex = index);
            tmpPartMen.Visible = false;
            tmpPartMen.Open();
            tempModsMenu = tmpPartMen;
        }else{
            var canContinue = true;
            if(item.Text == "Модель колёс"){
                if(WheelType == 10){
                    canContinue = false;
                    mp.gui.chat.push(`Вы не можете использовать этот тип колёс!`);
                }
                else if(vehClass == 8 && WheelType != 6){
                    canContinue = false;
                    mp.gui.chat.push(`Вы не можете использовать этот тип колеса на байке!`);
                }
                else if(vehClass != 8 && WheelType == 6){
                    canContinue = false;
                    mp.gui.chat.push(`Вы можете использовать этот тип колеса только на байке.!`);
                }
            }
            if(canContinue){
    ...
    let confrimationItem = new UIMenuItem("Купить", displayPrice);
    tmpPartMenu.AddItem(confrimationItem);
    bindConfirmationItmToMain(confrimationItem);
    tmpPartMenu.ItemSelect.on((item) => {
        if (item.Text == "Купить") {
...
function createColorMenu() {
    ...
    let lstColorOptions = [];
    let menuListItem;
    let colorPrice = getDisplayPrice("Цвет автомобиля");
    ...
    */
   let menModItem2 = new UIMenuItem("~r~Цвет автомобиля");
   menuVehColor.AddItem(menModItem2);

   menuListItem = new UIMenuItem("Основной цвет", colorPrice);
   menuVehColor.AddItem(menuListItem);

   menuListItem = new UIMenuItem("Дополнительный цвет", colorPrice);
   menuVehColor.AddItem(menuListItem);

   menuListItem = new UIMenuItem("Обычные цвета", colorPrice);
   menuVehColor.AddItem(menuListItem);

   menuListItem = new UIMenuItem("Цвет колёс", colorPrice);
   menuVehColor.AddItem(menuListItem);

    let confrimationItem = new UIMenuItem("Купить", colorPrice);
...
function createWheelColorMenu() {
    ...
    let lstColorOptions = [];
    let menuListItem;
    let colorPrice = getDisplayPrice("Цвет колёс");
    let player = mp.players.local;
    let currentPrimeryColor = player.vehicle.getColor(0);
    let currentSecondColor = player.vehicle.getColor(1);
    ...
    menuListItem = new UIMenuListItem("Цвет колёс", colorPrice, new ItemsCollection(lstColorOptions), currentPrimeryColor);
    menuVehColor.AddItem(menuListItem);

    menuVehColor.ListChange.on(wheelColorChangeEvent);

    let resetItem = new UIMenuItem("Сбросить цвет");
    ...
    menuVehColor.ItemSelect.on((item, index) => {
        if (item.Text == "Купить") {
            buyModIndex = mapModToIndex["Цвет колёс"];
            openConfrimationMenu();
            if (menuVehColor != null)
                menuVehColor.Visible = false;
        }else if (item.Text == "Сбросить цвет") {
...
function initModToPrice(serveArg) {
    ...
    let mapIndexToMod = {
        ...
        27: "Дизайн салона",
        28: "Украшения",
        30: "Дизайн приборной панели",
        33: "Руль",
        34: "Рычаг переключения передач",
        35: "Номера",
        38: "Гидравлика",
        48: "Ливерия",
        49: "Ливерия2",//FakeType for bugged livery functions
        62: "Номер",
        69: "Тонировка",
        999: "Цвет транспорта",
        9999: "Цвет транспорта (ex)",
        9998: "Цвет колес",
        1000: "Установка Неона",
        1001: "Неон",
        1002: "Экстра"
    };
...
function createModsMenus() {
    ...
    let menModItem2 = new UIMenuItem("~r~Модификация " + carName, "");
    ...
    for (let key_ModName in mapModToIndex) {
        ...
        if (vehicleModLimit > 0) {
            let menModItem = new UIMenuItem(key_ModName, vehicleModLimit + " опций доступно.");
    ...
    //Turbo
    mapModToIndex["Турбо"] = 18;
    let turbomenModItem = new UIMenuItem("Турбо", 1 + " опций доступно.");
    menuMods.AddItem(turbomenModItem);
    //menuMods.AddItem(menModItem);
    let turbotmpPartMen = createPartMenu("Турбо", 1, 0);
    ...
    //Xenon
    mapModToIndex["Ксенон"] = 22;
    let xenonmenModItem = new UIMenuItem("Ксенон", 1 + " опций доступно.");
    menuMods.AddItem(xenonmenModItem);
    //menuMods.AddItem(menModItem);
    let xenontmpPartMen = createPartMenu("Ксенон", 1, 0);
    ...
    mapModToIndex["Тонировка"] = 69;
    let windowsmenModItem = new UIMenuItem("Тонировка", 4 + " опций доступно.");
    ...
    mapModToIndex["Номер"] = 62;
/*let platesmenModItem = new UIMenuItem("Plates", 5 + " опций доступно.");
...
function getDisplayPrice(strPartName) {
    let displayPrice = "~r~Не доступно";

    //TODO: Add commas ","
    if (mapModToPrice[strPartName]) {
        displayPrice = "Цена: ~g~" + mapModToPrice[strPartName].toString() + "$";
    }
...
function endRental() {
	...
	mp.game.graphics.notify("[~y~Диспетчер такси~w~]: Пожалуйста, верните транспорт в ~y~таксопарк~w~.");
...
mp.events.add('render', (nametags) =>
{
    ...
    const showNametagId = mp.players.local.hasVariable('IDS_ARE_HIDDEN') && mp.players.local.getVariable('IDS_ARE_HIDDEN');
    var isPlayerTabbed = false;

    if (isLocalPlayerAOD){
        mp.vehicles.forEach(v =>
        {
            if(v && getDistanceBetweenCoords(mp.players.local.position, v.position) < 30.0 && v.dimension == mp.players.local.dimension && v != mp.players.local.vehicle)
            {
                var vehicleModel = v.getModel();
                mp.game.graphics.drawText("Model: " + mp.game.vehicle.getDisplayNameFromVehicleModel(vehicleModel) + "\nID: " + v.remoteId, [v.position.x, v.position.y, v.position.z + 1.5], { 
                    font: 4,
                    scale: [0.4, 0.4],
                    color: [255, 255, 255, 255],
                    outline: false
            ...
            if (isVisible)
            {
                ...
                if (distance <= maxDistance)
                {
                    ...
                    if (hasLOS)
                    {
                        ...
                        isPlayerTabbed = player.getVariable('PLAYER_IS_TABBED');
                        if (isPlayerTabbed == null) isPlayerTabbed = false;

                        if (isPlayerTabbed)
                            color2 = [142, 142, 142, 255];

                        var armour = player.getArmour() / 100;

                        y -= scale * (0.005 * (screenRes.y / 1080));

                        var nameTagText = player.name;
                        if (!showNametagId){
                            nameTagText = (isPlayerTabbed ? "AFK " : "") + player.name + " (" + ((player.getVariable('nametag_id') != -1) ? player.getVariable('nametag_id') : "X") + ")";
                            mp.game.graphics.drawText(nameTagText + (isLocalPlayerAOD ? ("\nHP: " + player.getHealth().toString() + " | AR: " + player.getArmour().toString()) : ""), [x, y + 0.03], {
                                font: 4,
                                color: color2,
                                scale: [0.4, 0.4],
                                outline: true,
                            ...
                            mp.game.graphics.drawText(nameTagText, [x, y + 0.03], {
                                font: 4,
                                color: color2,
                                scale: [0.4, 0.4],
                                outline: true,
...
mp.events.add('playerWeaponShot', (targetPosition, targetEntity) => {
    mp.events.callRemoteUnreliable("weapon_fired");
});
            ...
            if(mp.players.local.vehicle){
                if(isDriver() === true){
                    ...
                    if (IsModelBlocked(model)) {
                        mp.game.graphics.notify("~r~Невозможно использовать круиз-контроль на этом автомобиле!");
                        return;
                    }
...
mp.events.add('outOfTrunk', function (victim/*, veh*/) {
    // Remove blindfold
    ...
    victim.resetMovementClipset(0.0);
...
mp.events.add("render", () => {
    ...
    if(mp.game.controls.isDisabledControlJustPressed(0, 48) && !chatopened) {
    ...
    if(mp.game.controls.isDisabledControlJustPressed(0, 289)) {
...
mp.events.add(
{
    ...
    "blip_create_spec" : (name, position, color, type) => {
        BlipHelper.createBlipSpec(name, position, color, type);
    },
...
try {
if (player != undefined)        
player.clearTasksImmediately(); }catch (e) {}
    },
    "queue_moved_back" : () => {
        mp.events.call('destroyBrowser');
    },
	"account_prompt_login" : () => {
		...
		//Discord activity
        mp.discord.update('Играет на', 'GTA-WORLD.RU');
	...
	"account_prompt_creation" : (jailChar) => {
        mp.events.call('createBrowser', ['package://gtalife/login/character_creation.html', jailChar]);
	},
	...
	"send_player_dialogbox" : () => {
        cef_opened = true;
        cef = mp.browsers.new("package://gtalife/login/dialog_player.html");
	...
	"send_check_plate" : () => {
        cef_opened = true;
        cef = mp.browsers.new("package://gtalife/login/dialog_plate.html");
        mp.gui.cursor.show(true, true);
        mp.gui.chat.activate(false);
	},
	...
	"show_cef" : (url) => {
        mp.events.call('createBrowser', [url]);
        mp.game.graphics.notify("Используйте ~b~F4 ~w~чтобы закрыть интерфейс.");
	...
	"show_badge" : (url) => {
        mp.events.call('createBrowser', [url]);
        mp.game.graphics.notify("~b~F4 ~w~чтобы закрыть");
        mp.events.callRemote("cef_opened", true);
        mp.gui.cursor.show(false, false);
        mp.gui.chat.activate(true);
	},
	...
	"mute" : (admin) => {
        mp.gui.chat.activate(false);
        mp.game.graphics.notify("Вас заглушил Администратор " + admin);
	},
	...
	"unmute" : (admin) => {
        mp.gui.chat.activate(true);
        mp.game.graphics.notify("Вы были размучены администратором " + admin)
    },
...
anim_menu.AddItem(new UIMenuItem("~r~Выйти", "Close menu."));
...
anim_sub_menu.AddItem(new UIMenuItem("slutty9", "Animation")); // FemaleSlutty.
        ...
        anim_sub_menu.AddItem(new UIMenuItem("~r~Выйти", "Animation"));
 ...
 SetVehicleLimiterStatus2(vehicle, true);
    ...
    "radar_create_marker": (position, size) => {
        let player = mp.players.local;
        if (player !== null) {
            radar_Markers[player] = mp.markers.new(6, position, size,
            {
                direction: new mp.Vector3(0, 0, 0),
                rotation: new mp.Vector3(270, 0, 0),
                color: [0, 0, 200, 100],
                visible: true,
                dimension: 0
            });
...
mp.keys.bind(0x27, true, function() { // Right Indicator
    if (logged == 0 || chatopened  || cef_opened)
        return;
    let vehicle = mp.players.local.vehicle;
    if (vehicle && vehicle.getPedInSeat(-1) == mp.players.local.handle && blockedIndicatorClasses.indexOf(vehicle.getClass()) == -1) mp.events.callRemote("toggleIndicator", 0);
});
...
function handleLabelDataChange(entity, text, desc, checkForFoot) {
    ...
    var secondary_col = descriptionLabelColor;
    if (text.startsWith("(("))
       secondary_col = [182, 182, 184, 255];
    ...
    if (index == -1) {
        ...
        labelEmotes.push({tick: Date.now(), player: entity, text: text, color: (desc == true ? secondary_col : emoteLabelColor), destroyTimer: tempDestroyTimer, checkForFoot: checkForFoot })
    }
    else {
...
{
    let loca = mp.players.local;
    let aod = loca.getVariable("ON_DUTY_ADMIN");
    let tod = loca.getVariable("ON_DUTY_TESTER");

    if(aod === undefined && tod === undefined) return;

    if(aod)
    {
        mp.game.graphics.drawText("ADUTY", [0.98, 0.5], {
            font: 4,
            centre: true,
            color: [28, 128, 0, 255],
            outline: true,
    ...
    if(tod)
    {
        mp.game.graphics.drawText("TDUTY", [0.98, 0.6], {
            font: 4,
            centre: true,
            color: [115, 16, 16, 255],
            outline: true,
...
function reloadMic(){
    ...
    mp.game.graphics.notify('~g~Голосовой чат перезагружен');
}
...
function stopVehicle(player, vehicle) {
    ...
    mp.game.graphics.notify("Автопилот: ~r~Автопилот выключен.");
}
...
function autoPilotFeature(premiumLevel) {
    ...
    if (playerVehicle.getPedInSeat(-1) !== player.handle)
        return;
    ...
    if (!playerVehicle.getIsEngineRunning())
        return mp.game.graphics.notify("Автопилот: ~r~Двигатель не заведен.");

    // Check for a valid waypoint.
    var waypoint = getWaypointPos();
    if (waypoint.x == 0 && waypoint.y == 0 && waypoint.z == 0)
        return mp.game.graphics.notify("Автопилот: ~o~Укажите точку на карте.");

    mp.game.graphics.notify("Автопилот: ~g~Маршрут рассчитан.");
    ...
    if (!isDayTime)
        playerVehicle.setLights(0); // normal lights
    ...
    autopilot.interval = setInterval(() => {
        ...
        if (streetName.startsWith("Шоссе"))
        ...
        {
            stopVehicle(player, playerVehicle);
            player.taskVehicleTempAction(playerVehicle.handle, 27, 1e4); // Stop the vehicle.
            clearInterval(autopilot.interval);
            return;
        }
    }, 300);
...
mp.events.add("adminLightsMenu", (id, status) => {
    if (id === -1) {
        mp.game.graphics.resetLightsState();
        return;
    }

    mp.game.graphics.setLightsState(id, status);
});
