...
function CreateMenu() {
    poker_menu = new Menu("Ваш ход", "", new Point(posX, posY)); //
...
function RebuildMenu() {
    ...
    if (ourMoney > maxBet){
        ...
        poker_menu.AddItem(new UIMenuListItem("Рейз", "", new ItemsCollection(Amounts)));
    }
    if (maxBet > ourBet && ourMoney > 0) {
        poker_menu.AddItem(new UIMenuItem("Колл", ""));
    }else{
        poker_menu.AddItem(new UIMenuItem("Чек", ""));
    }
    if(SelfAllIn == false && IsBlindTurn == false)
        poker_menu.AddItem(new UIMenuItem("Фолд", ""));
}
...
mp.events.add(
{
    ...
    "SOMEONES_TURN": (player, name) => {
        ...
        currentTurn = player;
        removeLoadingMessage();
        startLoadingMessage ('STRING',"Ожидаем " + gamePlayers[player].name);
        myturn = false;
        updateTimerbars();
    },
    "SET_MAX_BET": (bet) => {
    ...
    "END_ROUND": () => {
        removeLoadingMessage();
        startLoadingMessage ('STRING',"Ожидаем начала новой игры");
    },
    "SKIP_TURN": () => {
