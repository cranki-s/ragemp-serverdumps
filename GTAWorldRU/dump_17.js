...
mp.keys.bind(0x52, true, (player) => {
    if (object_spawn !== "" && object_spawn != null) {
            ...
            {
                textList["objectstatus1"].textName = "Перемещение";
                object_spawn = "move_object";
            }
            else if (object_spawn === "move_object")
            {
                textList["objectstatus1"].textName = "Вращение";
...
mp.events.add("render", () => {
    ...
    if(mp.players.local.hasVariable("MANAGE_FURNITURE_CLICK_MODE") && mp.players.local.getVariable("MANAGE_FURNITURE_CLICK_MODE") == true) 
    {
        ...
        mp.game.graphics.drawText(`Управление мышью`, [0.5, 0.005], {
            font: 4,
        ...
        mp.game.graphics.drawText(`Вызвать курсор F3 | BAKSPACE для выхода ${debug ? `| ~r~DEBUG MODE` : ``}`, [0.5, 0.05], {
            font: 4,
            color: [255, 125, 0, 200],
            scale: [0.4, 0.4],
            outline: true,
