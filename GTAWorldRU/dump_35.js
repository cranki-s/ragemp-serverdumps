...
mp.events.add("startKeyPad", (title, subTitle, useAsterisk) => {
    ...
    mp.game.graphics.notify("Используйте '~g~CTRL~w~' или '~g~ПКМ~w~' чтобы закрыть меню.");
});
...
function CreateButtons() {
    ...
    var deleteButton = new KeypadButton(11, new Vector2(x, y - textOffset), new Vector2(x, y), new Vector2(0.13875, 0.09), "DEL");
    buttons.push(deleteButton);

    y = defaultYStartValue + (4 * defaultYOffset) + (4.5 * 0.09);
    var confirmButton = new KeypadButton(12, new Vector2(defaultPosition.X, y - textOffset), new Vector2(defaultPosition.X, y), new Vector2((0.13875 * 2) + defaultXOffset, 0.09), "Подтвердить");
...
mp.events.add("click", (x, y, upOrDown, leftOrRight) => {
    ...
    if (leftOrRight == "left" && upOrDown == "down") {
        if (keyPadObj.currentButton !== null && keyPadObj.text !== null)
        {
            ...
            if (keyPadObj.currentButton.text === "DEL") {
            ...
            else if (keyPadObj.currentButton.text === "Подтвердить") {
