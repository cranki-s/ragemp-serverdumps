...
var blacklist_items_nude = false;
...
mp.events.add(
{
    "OUTFITSHOP::SHOW" : (bool, prem = false, slot, creation = false) => {
        if (!mp.browsers.exists(outfitCEF))
        {
            outfitCEF = mp.browsers.new("package://gtalife/OutfitShop/index.html");
            mp.game.graphics.notify("~g~Майка под номером 15 является универсальной!");
            uniformStore = bool;
            blacklist_items_nude = prem;
            creationStore = creation;
            var SkinInt = mp.players.local.getModel();
            InitializeData();
            var data = JSON.stringify(outfitsData);
            outfitCEF.execute(`Initialize('${data}', ${uniformStore}, ${blacklist_items_nude}, '${wardrobe_names}', '${uniform_names}', ${slot}, ${creationStore}, ${SkinInt});`);
...
function InitializeData(){
    ...
    addOutfitData("рубашки", mp.players.local.getNumberOfDrawableVariations(11), mp.players.local.getNumberOfTextureVariations(11, mp.players.local.getDrawableVariation(11)), mp.players.local.getDrawableVariation(11), mp.players.local.getTextureVariation(11), 11, 0);
    addOutfitData("торсы", mp.players.local.getNumberOfDrawableVariations(3), mp.players.local.getNumberOfTextureVariations(3, mp.players.local.getDrawableVariation(3)), mp.players.local.getDrawableVariation(3), mp.players.local.getTextureVariation(3), 3, 0);
    addOutfitData("штаны", mp.players.local.getNumberOfDrawableVariations(4), mp.players.local.getNumberOfTextureVariations(4, mp.players.local.getDrawableVariation(4)), mp.players.local.getDrawableVariation(4), mp.players.local.getTextureVariation(4), 4, 0);
    addOutfitData("обувь", mp.players.local.getNumberOfDrawableVariations(6), mp.players.local.getNumberOfTextureVariations(6, mp.players.local.getDrawableVariation(6)), mp.players.local.getDrawableVariation(6), mp.players.local.getTextureVariation(6), 6, 0);
    addOutfitData("майки", mp.players.local.getNumberOfDrawableVariations(8), mp.players.local.getNumberOfTextureVariations(8, mp.players.local.getDrawableVariation(8)), mp.players.local.getDrawableVariation(8), mp.players.local.getTextureVariation(8), 8, 0);
    addOutfitData("аксессуары", mp.players.local.getNumberOfDrawableVariations(7), mp.players.local.getNumberOfTextureVariations(7, mp.players.local.getDrawableVariation(7)), mp.players.local.getDrawableVariation(7), mp.players.local.getTextureVariation(7), 7, 0);
    addOutfitData("наклейки", mp.players.local.getNumberOfDrawableVariations(10), mp.players.local.getNumberOfTextureVariations(10, mp.players.local.getDrawableVariation(10)), mp.players.local.getDrawableVariation(10), mp.players.local.getTextureVariation(10), 10, 0);
    addOutfitData("шляпы", mp.players.local.getNumberOfPropDrawableVariations(0), mp.players.local.getNumberOfPropTextureVariations(0, mp.players.local.getPropIndex(0)), mp.players.local.getPropIndex(0), mp.players.local.getPropTextureIndex(0), 0, 1);
    addOutfitData("очки", mp.players.local.getNumberOfPropDrawableVariations(1), mp.players.local.getNumberOfPropTextureVariations(1, mp.players.local.getPropIndex(1)), mp.players.local.getPropIndex(1), mp.players.local.getPropTextureIndex(1), 1, 1);
    addOutfitData("уши", mp.players.local.getNumberOfPropDrawableVariations(2), mp.players.local.getNumberOfPropTextureVariations(2, mp.players.local.getPropIndex(2)), mp.players.local.getPropIndex(2), mp.players.local.getPropTextureIndex(2), 2, 1);
    addOutfitData("значки", mp.players.local.getNumberOfDrawableVariations(5), mp.players.local.getNumberOfTextureVariations(5, mp.players.local.getDrawableVariation(5)), mp.players.local.getDrawableVariation(5), mp.players.local.getTextureVariation(5), 5, 0);
    addOutfitData("броня", mp.players.local.getNumberOfDrawableVariations(9), mp.players.local.getNumberOfTextureVariations(9, mp.players.local.getDrawableVariation(9)), mp.players.local.getDrawableVariation(9), mp.players.local.getTextureVariation(9), 9, 0);
}
...
function PurchaseOutfit(slot,name){
