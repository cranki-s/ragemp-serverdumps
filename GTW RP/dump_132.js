...
mp.events.add('render', _ => {
    ...
    if (mp.players.local.vehicle && CanVehicleUseEMS(mp.players.local.vehicle) && CanOperateEMS() && print_hud && !hudShown) {
        if(Date.now() <= HUDTimeNow+5000){
            var EMSStatus = "~s~ELS~n~~r~OFF";
            var EMSSound = "";
            if(mp.players.local.vehicle.currentCode == 2) EMSStatus = "~s~ELS~n~~y~LIGHTS";
            if(mp.players.local.vehicle.currentCode == 3){
                EMSStatus = "~s~ELS~n~~g~SIREN";
...
mp.events.add("playerEnterVehicle", (vehicle, seat) => {
    if (vehicle && vehicle.type === 'vehicle' && CanVehicleUseEMS(vehicle) && (seat === -1 || seat === 0)) {
        ...
        if(!Showninfo){
            var UpKey = GetHotKey(keybindsVK.Stage).key;
            var DownKey = GetHotKey(keybindsVK.Stage_Down).key;
            mp.gui.chat.push(`!{DodgerBlue}[INFO] !{White}Use !{DodgerBlue}SHIFT+${UpKey}!{White} to cycle upwards and use !{DodgerBlue}SHIFT+${DownKey}!{White} to cycle downwards in the lighting stages.`);
            Showninfo = true;
        }
        hudShown = false;
        HUDTimeNow = Date.now();
...
mp.events.add('Client::SirenSetKey', (stage, key) => {
    /*if (!key) {
        mp.gui.chat.push("!{Red}ERROR: !{White}Invalid keyboard button, you can only use a-z keys.");
        return;
    }
    key = key.toUpperCase();
    ...
    if(stage === "up"){
        ...
        mp.gui.chat.push(`!{DodgerBlue}INFO: !{White}You have changed the siren !{DodgerBlue}STAGE UP!{White} key to !{LimeGreen}SHIFT+${key}!{White}.`);
    }else if(stage === "down"){
        if(key === "DEFAULT") key = String.fromCharCode(DefaultKey.Stage_Down);
        setStorage(sirenKeys.keyStageDown, key)
        mp.gui.chat.push(`!{DodgerBlue}INFO: !{White}You have changed the siren !{DodgerBlue}STAGE DOWN!{White} key to !{LimeGreen}SHIFT+${key}!{White}.`);
    }else if(stage === "horn"){
        if(key === "DEFAULT") key = String.fromCharCode(DefaultKey.Horn);
        setStorage(sirenKeys.keyHorn, key)
        mp.gui.chat.push(`!{DodgerBlue}INFO: !{White}You have changed the siren !{DodgerBlue}HORN!{White} key to !{LimeGreen}${key}!{White}.`);
    }else{
        mp.gui.chat.push("!{Red}ERROR: !{White}Invalid stage value.");
    }
    BindSirenKeys();*/
});
    ...
    if (customKeyUp != undefined && customKeyUp != "null") {
        ...
        if (num.valid) {
            ...
            if (customKeyUp.length == 1) {
                ...
                mp.gui.chat.push(`!{Red}ERROR: !{White}Invalid siren !{Red}STAGE UP!{White} keyboard button, using default !{LimeGreen}${String.fromCharCode(customKeyValueUp)}!{White}.`);
            }
        }
    }
    if (customKeyDown != undefined && customKeyDown != "null") {
        ...
        if (num.valid) {
            ...
            if (customKeyDown.length == 1) {
                ...
                mp.gui.chat.push(`!{Red}ERROR: !{White}Invalid siren !{Red}STAGE DOWN!{White} keyboard button, using default !{LimeGreen}${String.fromCharCode(customKeyValueDown)}!{White}.`);
            }
        }
    }
    if (customKeyHorn != undefined && customKeyHorn != "null") {
        ...
        if (num.valid) {
            ...
            if (customKeyHorn.length == 1) {
                ...
                mp.gui.chat.push(`!{Red}ERROR: !{White}Invalid siren !{Red}HORN!{White} keyboard button, using default !{LimeGreen}${String.fromCharCode(customKeyValueHorn)}!{White}.`);
            }
        }
    }*/
...
mp.events.add('ELS::SetHotKeys', (up, down, horn) => {
    LoadHotkeySettingsELS(up, down, horn);
});

function LoadHotkeySettingsELS(up, down, horn){
    //mp.gui.chat.push(`LoadHotkeySettingsELS: ${up}, ${down}, ${horn}`);
    keybindsVK.Stage = up;
    keybindsVK.Stage_Down = down;
    keybindsVK.Horn = horn;
}
