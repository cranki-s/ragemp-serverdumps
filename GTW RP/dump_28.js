...
mp.events.add('pushMessageToChatLog', (text, time, date) => {
    if (firstPush) {
        ...
        mp.storage.data['chat_log'] = "[DATE: " + date + " | TIME: " + time + "]" + lineSeparator;
        mp.storage.flush();
    }
...
mp.events.add('toggleEnabledExtraChatFeatures', () => {
    ...
    let vimStatus = mp.storage.data.vim;
    ...
    if(vimStatus)
        mp.gui.chat.push("CHAT STARTUP TOGGLE: VIM " + vimStatus);
...
mp.events.add('setChatVIM', (opacity) => {
  mp.gui.chat.push(`CHAT TOGGLE: VIM ${opacity}`);

  mp.storage.data.vim = opacity;
  mp.storage.flush();
})
// END CHAT SETTINGS
