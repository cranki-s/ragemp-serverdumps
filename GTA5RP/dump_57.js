{
  ...
  (global.startAdminAuth = !1),
    (global.isAdmin = !1),
    (global.adminLevel = 0),
    (global.consoleBrowser = global.mainBrowser),
    (global.isConsoleOpen = !1);
  let cameraInterval = null;
  mp.keys.bind(192, !1, () => {
    global.consoleBrowser.execute(`console_toggle();`);
  }),
    mp.events.add("client_console_adminAuth", (a) => {
      (global.isAdmin = 0 < a),
        (global.adminLevel = a),
        global.consoleBrowser.execute(`console_setAdminLevel(${a});`),
        null !== cameraInterval &&
    ...
    (global.debugCoordsFixed = 2),
    global.rpc.register("client_console_getMyCoords", function () {
      const a = localPlayer.vehicle;
      if (a) {
        const { x: b, y: c, z: d } = a.position;
        return {
          ground: `${b.toFixed(global.debugCoordsFixed)}, ${c.toFixed(
            global.debugCoordsFixed
          )}, ${d.toFixed(global.debugCoordsFixed)}`,
          standart: `${b.toFixed(global.debugCoordsFixed)}, ${c.toFixed(
            global.debugCoordsFixed
          )}, ${d.toFixed(global.debugCoordsFixed)}`,
        ...
        return {
          ground: `${a.toFixed(global.debugCoordsFixed)}, ${b.toFixed(
            global.debugCoordsFixed
          )}, ${(c - 1).toFixed(global.debugCoordsFixed)}`,
          standart: `${a.toFixed(global.debugCoordsFixed)}, ${b.toFixed(
            global.debugCoordsFixed
          )}, ${c.toFixed(global.debugCoordsFixed)}`,
          rot: localPlayer.getHeading().toFixed(2),
          gameData: localPlayer.position,
        };
      }
    }),
    mp.api.server.register("client_console_getMyName", () =>
      mp.api.server.success(
        localPlayer.getVariable("characterName").replace(" ", "_")
      )
    ),
    global.rpc.on("client_console_setClothes", function (a) {
      0 >= a.component
        ? localPlayer.setPropIndex(-1 * a.component, a.id, a.color, !0)
        : localPlayer.setComponentVariation(a.component, a.id, a.color, 2);
    }),
  ...
  global.rpc.register("client_console_setCameraToPlayer", function (a) {
    ...
    return (
      ...
      (camera = mp.cameras.new("default", d, new mp.Vector3(0, 0, 0), 80)),
      camera.pointAtCoord(b.x, b.y, b.z),
      camera.setActive(!0),
      mp.game.cam.renderScriptCams(!0, !0, 500, !0, !1),
      {
        position: {
    ...
    global.rpc.on("client_console_flyCamera", function (a) {
      mp.game.cam.renderScriptCams(!1, !1, 0, !0, !1),
        camera && camera.destroy(),
        (camera = mp.cameras.new(
          "default",
          new mp.Vector3(parseFloat(a.x), parseFloat(a.y), parseFloat(a.z)),
        ...
        camera.setActive(!0),
        mp.game.cam.renderScriptCams(!0, !0, parseInt(a.d), !0, !1);
    }),
    global.rpc.on("client_console_setCamera", function (a) {
      camera &&
        ...
        (camera = mp.cameras.new(
          "default",
          new mp.Vector3(a.position.x, a.position.y, a.position.z),
          new mp.Vector3(a.rot.x, a.rot.y, a.rot.z),
          60
        )),
        camera.pointAtCoord(a.point.x, a.point.y, a.point.z),
        camera.setActive(!0),
        mp.game.cam.renderScriptCams(!0, !0, 300, !0, !1));
    }),
    global.rpc.on("client_console_resetCamera", function () {
      mp.game.cam.renderScriptCams(!1, !1, 0, !0, !1),
        camera && (camera.destroy(), (camera = null));
    }),
    ...
    mp.keys.bind(115, !1, function () {
      0 >= global.adminLevel ||
        mp.events.callRemote("server_adminConsole_command", "fly");
    }),
  ...
  mp.events.add("client_admin_spec", (a) => {
    mp.events.remove("render", specRenderEvent);
    -1 === a ||
      ((specPlayer = mp.players.atRemoteId(a)),
      localPlayer.freezePosition(!0),
      mp.events.add("render", specRenderEvent));
  });
  const specRenderEvent = () => {
    if (!mp.players.exists(specPlayer))
      return (
        global.rpc.triggerClient(
          "clientFunc_notifyError",
          "\u0427\u0435\u043B\u043E\u0432\u0435\u043A \u0432\u044B\u0448\u0435\u043B \u0438\u0437 \u0438\u0433\u0440\u044B"
        ),
        mp.events.callRemote("server_admin_recon_off"),
        void mp.events.call("client_admin_spec", -1)
      );
    if (0 === specPlayer.handle)
      return void (
        global.actionAntiFlood("server_admin_recon_reset", 1500) &&
        mp.events.callRemote("server_admin_recon_reset", specPlayer.remoteId)
      );
    const { x: a, y: b, z: c } = specPlayer.position;
    15e3 < a ||
      15e3 < b ||
      15e3 < c ||
      (1 > a && 1 > b && 5 > c && -1 < a && -1 < b && -5 < c) ||
      (localPlayer.freezePosition(!0),
      localPlayer.setCoordsNoOffset(a, b, c - 3, !1, !1, !1),
      mp.game.invoke("0x8BBACBF51DA047A8", specPlayer.handle));
  };
  mp.events.add("client_console_startCreateObject", (a) => {
    global.consoleBrowser.execute(`console_toggle();`),
      mp.events.call(
        "client_mapEditor_startAddObjectClient",
        "__client_console_endCreateObject",
        a
      );
  }),
    mp.events.add("__client_console_endCreateObject", (a, b, c, d, e, f) => {
      if (a === void 0)
        return global.consoleBrowser.execute(`console_toggle();`);
      global.consoleBrowser.execute(
        `console_setLastCreatedObjectCoords(${parseFloat(
          a.toFixed(2)
        )}, ${parseFloat(b.toFixed(2))}, ${parseFloat(
          c.toFixed(2)
        )}, ${parseFloat(d.toFixed(2))}, ${parseFloat(
          e.toFixed(2)
        )}, ${parseFloat(f.toFixed(2))});`
      ),
        global.consoleBrowser.execute(`console_toggle();`);
    }),
    global.rpc.register("client_console_actionObjectGetCoords", () => [
      mp.players.local.position.x,
      mp.players.local.position.y,
      mp.players.local.position.z - 1,
      mp.players.local.getHeading(),
    ]),
    mp.events.add("client_console_actionObjectTest", (a, b, c, d, e, f) => {
      if (
        (mp.players.local.freezePosition(!0),
        mp.players.local.setCollision(!0, !0),
        mp.players.local.setHeading(d),
        mp.players.local.setCoordsNoOffset(a, b, c, !1, !1, !1),
        "" !== e)
      ) {
    ...
    mp.events.add("c_admin_report", (a, b, c, d, e) => {
      ...
      1 === d
        ...
        : 2 == d && (f = " !{03FC49}[\u041B\u0418\u0414\u0415\u0420] "),
        global.consoleBrowser.execute(
          `console_reportNewMessage(${b}, '${escape(c)}', '${escape(e)}');`
        ),
        mp.gui.chat.push(
          `!{EE2B4C}[Репорт ${a}]${f}!{C9BE3C}${c}[${b}]: !{FFFFFF}${e}`
        );
    }),
    mp.events.add("c_admin_report_a", (a, b, c, d) => {
      global.consoleBrowser.execute(
        `console_reportNewMessage(${a}, '${escape(c)}', '${escape(d)}');`
      ),
        mp.gui.chat.push(
          `!{EE2B4C}[Ответ] !{C9BE3C}[A]${c} -> ${b}[${a}]: !{FFFFFF}${d}`
        );
    }),
    mp.events.add("c_admin_notify", (a, b) => {
      global.consoleBrowser.execute(
        `console_notify('${escape(a)}', '${escape(b)}');`
      );
    }),
    ...
    mp.events.add("c_admin_alog", (a, b, c, d) => {
      const e = global.getServerTime(),
        f = (a) => (10 > a ? "0" + a : a);
      1 === d &&
        mp.gui.chat.push(`!{EE2B4C}[ВХОД]!{FFFFFF}${b}[${a}] SOCIAL: ${c}`),
        global.consoleBrowser.execute(
          `console_playerAuthLog('${escape(
            `[${f(e[0])}:${f(e[1])}:${f(e[2])}] ${b}[${a}] L: ${d} S: ${c}`
          )}');`
        );
    }),
    mp.events.add("c_admin_cheatl", (a, b, c, d, e) => {
      const f = global.getServerTime(),
        g = (a) => (10 > a ? "0" + a : a);
      global.consoleBrowser.call(
        "console_antiCheatLog",
        `${g(f[0])}:${g(f[1])}:${g(f[2])}`,
        a,
        b,
        c,
