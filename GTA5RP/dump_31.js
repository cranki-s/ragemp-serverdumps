{
  ...
  mp.events.add("entityStreamIn", function (a) {
    ...
    mp.events.add("client_playerBlip_create", function (a, b) {
      ...
      null == c ||
        (0 !== c.handle &&
          (playerBlipList.has(a) && 0 !== c.blip
            ? c.setBlipColor(isNaN(b) ? 0 : b)
            : (0 == c.blip && c.createBlip(1),
              c.setBlipColor(isNaN(b) ? 0 : b))));
    }),
    mp.events.add("client_playerBlip_destroy", function (a) {
      const b = mp.players.atRemoteId(a);
      return null == b || 0 === b.handle
        ? playerBlipList.delete(a)
        : void (playerBlipList.has(a) && 0 != b.blip && b.destroyBlip(),
          playerBlipList.delete(a));
    }),
