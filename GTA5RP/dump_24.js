{
    ...
    global.fingerPointingEnableOutSync = !0, global.fingerPointingActive = !1, mp.events.add("fpsync.update", async (a, b, c) => {
        ...
        if (d && 0 !== d.handle && d != mp.players.local) {
            if (d.lastReceivedPointing = Date.now(), !d.pointingInterval) {
                ...
                }.bind(d), 500), mp.game.streaming.requestAnimDict("anim@mp_point");
    ...
    const fingerBinder = global.binder.register({
        ...
        defaultKey: 78,
        func: () => {
            mp.gui.cursor.visible || global.isChatOpen || global.disableKeys || global.isConfirmMenuOpen || mp.players.local.getVariable("cuffed") || 0 != global.getPlayerCurrentWeaponData().weapon || global.isPlayerDeath || fingerInterval || (pointing.start(), global.fingerPointingActive = !0, fingerInterval = setInterval(() => {
                mp.keys.isDown(fingerBinder.key) || mp.keys.isDown(fingerBinder.key) || (global.fingerPointingActive = !1, pointing.stop(), clearInterval(fingerInterval), fingerInterval = null)
            }, 150))
        }
    });
}