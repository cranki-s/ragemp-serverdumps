{
    ...
    global.binder.register({
        ...
        func: async () => {
            const _ = Date.now();
            if (mp.gui.cursor.visible || global.isChatOpen || global.disableKeys || global.isSmartphoneOpen || global.disableAnimList || localPlayer.vehicle || 2 < localPlayer.getSpeed() || localPlayer.getIsTaskActive(167) || lastScenarioUsed + 1e3 > _) return;
            const a = localPlayer.position,
                e = !mp.game.interior.areCoordsCollidingWithExterior(a.x, a.y, a.z);
            if (isPlayerInScenario) return 10 > mp.dist(a.x, a.y, a.z, lastScenarioPos.x, lastScenarioPos.y, lastScenarioPos.z) && localPlayer.clearTasks(), isPlayerInScenario = !1, void(lastScenarioUsed = _);
            ...
            if (!A) {
                ...
                if (o = s[1], A = SEAT_DATA.find(_ => _.objectHash === o[0]), !A) return
            }
            let t = null,
                H = null,
                c = 3;
            for (const _ of A.seats) {
                const e = mp.game.object.getObjectOffsetFromCoords(o[1].x, o[1].y, o[1].z, o[2].z, _.x, _.y, _.z),
            ...
            if (t && H) {
                ...
                for (const _ of s)
                    if (.5 > mp.dist(H.x, H.y, H.z, _.x, _.y, _.z) && (t.scenario && _.player.isUsingScenario(t.scenario) || t.animDict && _.player.isPlayingAnim(t.animDict, t.animName, 3))) return;
                if (t.scenario) localPlayer.taskStartScenarioAtPosition(t.scenario, H.x, H.y, H.z, o[2].z + t.h, 0, !0, e);
                else if (t.animDict) {
                    if (!mp.game.streaming.hasAnimDictLoaded(t.animDict)) {
                        mp.game.streaming.requestAnimDict(t.animDict);
                        do await mp.game.waitAsync(10); while (!mp.game.streaming.hasAnimDictLoaded(t.animDict))
                    }
                    localPlayer.setCoordsNoOffset(H.x, H.y, H.z, !0, !0, !0), localPlayer.setHeading(o[2].z + t.h), localPlayer.taskPlayAnimAdvanced(t.animDict, t.animName, H.x, H.y, H.z, 0, 0, o[2].z + t.h, 8, -8, -1, 513, 0, 0, 0)
                }
                isPlayerInScenario = !0, lastScenarioPos = H, lastScenarioUsed = _
            }
        }
    });
    let leaningTimer = null,
        isLeaningTaskActive = !1,
        isLeaningCancel = !1,
        lastLeaninglUse = -1;
    const leaningBinder = global.binder.register({
        action: "MAP_UNIVERSAL_LEANING",
        desc: "\u041F\u0440\u0438\u0441\u043B\u043E\u043D\u0438\u0442\u044C\u0441\u044F (\u0437\u0430\u0436\u0430\u0442\u044C)",
        defaultKey: -1,
        func: () => {
            var _ = Math.pow;
            if (mp.gui.cursor.visible || global.isChatOpen || global.disableKeys || global.isPlayerDeath || global.disableAnimList || global.isAnyPedAttachedToLocalPlayer || global.isPlayerUseAnim || null !== localPlayer.vehicle || localPlayer.getVariable("cuffed") || localPlayer.isSwimming() || localPlayer.isSwimmingUnderWater() || localPlayer.isReloading() || lastLeaninglUse + 500 > new Date().getTime() || leaningTimer) return;
            if (isLeaningTaskActive) return void(isLeaningCancel = !0);
            let a = null,
                e = null;
            const s = (_, a) => {
                    const e = localPlayer.getBoneCoords(12844, .5, 0, 0),
                        s = mp.game.graphics.getScreenActiveResolution(1, 1),
                        o = mp.game.graphics.screen2dToWorld3d([s.x / 2, s.y / 2, 14]);
                    return o ? mp.raycasting.testPointToPoint(e, o, _, a) : void 0
                },
                o = (a, e) => Math.sqrt(_(a.x - e.x, 2) + _(a.y - e.y, 2) + _(a.z - e.z, 2));
            leaningTimer = setInterval(() => {
                const _ = localPlayer.getCoords(!0),
                    A = localPlayer.getVariable("cuffed") || global.isPlayerDeath || localPlayer.isRagdoll() || global.disableAnimList || global.isPlayerUseAnim;
                if (lastLeaninglUse + 500 < new Date().getTime() && !mp.keys.isDown(leaningBinder.key) && !mp.keys.isDown(leaningBinder.key) || A) {
                    if (!A && e && 3.5 > o(_, e)) {
                        const a = mp.game.gameplay.getHeadingFromVector2d(_.x - e.x, _.y - e.y),
                            s = mp.game.gameplay.getGroundZFor3dCoord(e.x, e.y, e.z, 0, !1),
                            o = 4e3;
                        isLeaningTaskActive = !0, isLeaningCancel = !1, (async () => {
                            let _ = 0,
                                A = Date.now();
                            const t = setInterval(() => {
                                if (Date.now() > A + o || isLeaningCancel) return localPlayer.getVariable("cuffed") || global.isPlayerDeath || localPlayer.isRagdoll() || global.isPlayerUseAnim || localPlayer.vehicle || localPlayer.clearTasks(), isLeaningTaskActive = !1, void clearInterval(t);
                                switch (_) {
                                    case 0: {
                                        localPlayer.taskGoStraightToCoord(e.x, e.y, s, 1.1, o, a, .25), _++;
                                        break
                                    }
                                    case 1: {
                                        if (localPlayer.getHeading().toFixed(2) != a.toFixed(2) || localPlayer.getVariable("cuffed") || global.isPlayerDeath || localPlayer.isRagdoll() || global.disableAnimList || global.isPlayerUseAnim) return;
                                        _++;
                                        break
                                    }
                                    case 2: {
                                        localPlayer.taskStartScenarioInPlace("WORLD_HUMAN_LEANING", -1, !0), mp.events.call("c:anim:startGameScenario"), isLeaningTaskActive = !1, clearInterval(t);
                                        break
                                    }
                                }
                            }, 0)
                    ...
                    return clearInterval(leaningTimer), leaningTimer = null, void(lastLeaninglUse = new Date().getTime())
                }
                if (a = s(localPlayer, 19), !a || 3.5 <= o(_, a.position)) return void(e = null);
                if (a.position.z < _.z - .95) return a = null, void(e = null);
                const t = mp.game.gameplay.getHeadingFromVector2d(_.x - a.position.x, _.y - a.position.y),
                    H = new mp.api.DirectionVector(a.position, {
                        x: 0,
                        y: 0,
                        z: t
                    });
                e = H.Forward(a.entity && mp.vehicles.exists(a.entity) ? .2 : .3), mp.game.graphics.drawMarker(1, e.x, e.y, _.z - .85, 0, 0, 0, 0, 180, 0, .25, .25, .25, 3, 168, 83, 200, !1, !0, 2, !1, null, null, !1)
            }, 0), lastLeaninglUse = new Date().getTime()
        }
    });
}