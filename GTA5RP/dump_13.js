{
  mp.attachmentMngr = {
    ...
    addFor: async function (a, b) {
      if (
        this.attachments.hasOwnProperty(b) &&
        null != a.__attachmentObjects &&
        !a.__attachmentObjects.hasOwnProperty(b)
      ) {
        const c = this.attachments[b];
        if (c.weaponHash) {
          const d = global.playerWeaponGetModsData(a, c.weaponHash),
            e = WEAPON_ATTACH_REPLACE_DATA.find((a) =>
              d.find((b) => b.mod.mod === a[0])
            ),
            f = e
              ? mp.objects.new(e[1], a.position, { dimension: -1 })
              : mp.objects.newWeaponObject(c.weaponHash, a.position, {
                  ammo: 1,
                  scale: 1,
                  showWorldObject: !0,
          ...
          if (!f) return;
          for (let a = 0; 0 === f.handle && 2e3 > a; ++a)
          ...
          if (
            ...
            return void (mp.objects.exists(f) && f.destroy());
          if (
            (f.attachTo(
              a.handle,
              "string" == typeof c.boneName
                ? a.getBoneIndexByName(c.boneName)
                : a.getBoneIndex(c.boneName),
              c.offset.x,
              c.offset.y,
              c.offset.z,
              c.rotation.x,
            ...
            for (const a of d) a.mod.setToObject(f.handle, a.param);
          return void (a.__attachmentObjects[b] = f);
        }
        const d = mp.objects.new(c.model, a.position, { dimension: -1 });
        if (!d) return;
        for (let a = 0; 0 === d.handle && 2e3 > a; ++a)
          await mp.game.waitAsync(0);
        if (
          !mp.objects.exists(d) ||
          !mp.players.exists(a) ||
          ...
          return void (mp.objects.exists(d) && d.destroy());
        d.attachTo(
          a.handle,
          "string" == typeof c.boneName
            ? a.getBoneIndexByName(c.boneName)
          ...
          (a.__attachmentObjects[b] = d);
      }
    },
    removeFor: function (a, b) {
      if (
        a.__attachmentObjects != null &&
        a.__attachmentObjects.hasOwnProperty(b)
      ) {
    ...
    register: function (a, b, c, d, e) {
      "string" == typeof a && (a = mp.game.joaat(a)),
        "string" == typeof b && (b = mp.game.joaat(b)),
        mp.game.streaming.isModelInCdimage(b) &&
    ...
    unregister: function (a) {
      "string" == typeof a && (a = mp.game.joaat(a)),
        this.attachments.hasOwnProperty(a) && (this.attachments[a] = void 0);
    },
    getAttachments: function () {
      return Object.assign({}, this.attachments);
    },
    addClient: function (a, b) {
      a.__attachmentObjects == null && (a.__attachmentObjects = {}),
        this.addFor(a, b);
    },
    addClientByFakeId: async function (a, b, c) {
      a.__attachmentObjects == null && (a.__attachmentObjects = {}),
        await this.addFor(a, b),
        a.__attachmentObjects[b] &&
          ((a.__attachmentObjects[c] = a.__attachmentObjects[b]),
          delete a.__attachmentObjects[b]);
    },
    addToPed: async function (a, b) {
      if (
        this.attachments.hasOwnProperty(b) &&
        (null == a.__attachmentObjects && (a.__attachmentObjects = {}),
        !a.__attachmentObjects.hasOwnProperty(b))
      ) {
        const c = this.attachments[b],
          d = mp.objects.new(
            c.model,
            new mp.Vector3(a.position.x, a.position.y, a.position.z - 5),
            { dimension: -1 }
          );
        if (!d) return;
        for (let a = 0; 0 === d.handle && 1e4 > a; ++a)
          await mp.game.waitAsync(0);
        if (
          !mp.objects.exists(d) ||
          !mp.peds.exists(a) ||
  ...
  mp.objects.newWeaponObject || (mp.objects.newWeaponObject = () => null),
    ...
    mp.events.addDataHandler("attachmentsData", function (a, b) {
      if (0 !== a.handle) {
        ...
        if (0 !== a.handle) {
          ...
          for (let d of b)
            -1 === c.indexOf(d) && mp.attachmentMngr.removeFor(a, d);
          for (let d of c)
            -1 === b.indexOf(d) && mp.attachmentMngr.addFor(a, d);
        }
        a.__attachments = c;
      }
    }),
    ...
    mp.attachmentMngr.register(
      ...
      "prop_spraygun_01",
      28422,
      new mp.Vector3(0.01, -0.11, -0.08),
      new mp.Vector3(-35, 0, 2)
    ),
    mp.attachmentMngr.register(
      "WaterIngcan",
      "prop_wateringcan",
      28422,
      new mp.Vector3(0.26, -0.1, -0.12),
      new mp.Vector3(-50, 0, 2)
    ),
    mp.attachmentMngr.register(
      "flowers1",
      "cls_flower_buket_calla",
      60309,
      new mp.Vector3(0.05, 0.03, 0.03),
      new mp.Vector3(50, 190, 0)
    ),
    mp.attachmentMngr.register(
      "flowers2",
      "cls_flower_buket_florist",
      60309,
      new mp.Vector3(0.05, 0.03, 0.03),
      new mp.Vector3(50, 190, 0)
    ),
    ...
    mp.attachmentMngr.register(
      "pumpkin_hat",
      "cls_hw_gourd_small_hand",
      12844,
      new mp.Vector3(0.04, 0, 0),
