{
    ...
    global.isGameBrowserOpen = !1, global.rpc.on("client_rpBrowser_open", function () {
        global.isGameBrowserOpen || global.rpc.triggerBrowser(browserBrowser, "client_browser_rpBrowser_open")
    }), global.rpc.on("client_rpBrowser_close", function () {
        global.isGameBrowserOpen && global.rpc.triggerBrowser(browserBrowser, "client_browser_rpBrowser_close")
    }), global.rpc.register("client_rpBrowser_getPlayerData", function () {
        return {
            playerFaction: global.getEntityVariable(localPlayer, "factionId", ""),
            playerFactionType: global.getEntityVariable(localPlayer, "factionType", -1)
        }
    });
    ...
    global.rpc.register("__client_casinoRoulette_data", async () => {
        try {
            if (null === casinoRouletteData) {
                ...
                return casinoRouletteData = a.map(a => ({
                    id: a[0],
                    name: a[1],
                    desc: a[2],
                    image: a[3],
    ...
    }), mp.events.add("__client_menu_browserOpenTabSite", a => {
        global.isGameBrowserOpen ? browserBrowser.execute(`window.mainBrowser.openNewTab('${a}')`) : browserBrowser.execute(`browserOpen(); window.mainBrowser.openNewTab('${a}')`)
    });
    let fiveVitoFavoriteList = [];
    global.rpc.register("__client_5vito_fList_get", () => fiveVitoFavoriteList), global.rpc.register("__client_5vito_fList_add", a => {
        fiveVitoFavoriteList.push(a)
    }), global.rpc.register("__client_5vito_fList_delete", a => {
        fiveVitoFavoriteList = fiveVitoFavoriteList.filter(b => b.uId !== a.uId)
    }), global.rpc.on("__client_5vito_startTestDrive", async a => {
        const b = mp.vehicles.new(mp.game.joaat(a.model), mp.players.local.position, {
            dimension: mp.players.local.remoteId + 1
        });
        for (let c = 0; 0 === b.handle && 1e4 > c; ++c) await mp.game.waitAsync(0);
        const c = setInterval(() => {
            mp.vehicles.exists(b) && global.mainBrowser.execute(`mainHud.vehicleData.speed = ${Math.round(3.6*b.getSpeed())};`)
        }, 250);
    ...
    `);
        let d = 0,
            e = -1,
            f = 1;
        const g = () => !mp.vehicles.exists(b) || 2e3 < ++d && mp.players.local.vehicle != b ? (mp.events.remove("render", g), clearInterval(c), mp.vehicles.exists(b) && b.destroy(), global.mainBrowser.execute(`mainHud.vehicleData.enable = false;`), void mp.events.callRemote("server_5vito_endVehTestDrive")) : void(-1 !== e && b.setMaxSpeed(e));
        setTimeout(() => {
            if (0 !== b.handle) {
                e = -1, f = 1, b.setCustomPrimaryColour(a.color[0][0], a.color[0][1], a.color[0][2]), b.setCustomSecondaryColour(a.color[1][0], a.color[1][1], a.color[1][2]), b.setModColor1(a.colorType[0], 0, 0), b.setModColor2(a.colorType[1], 0), b.setExtraColours(a.pColor, a.wColor), -1 !== a.neon[0] && (b.setNeonLightsColour(a.neon[0], a.neon[1], a.neon[2]), b.setNeonLightEnabled(0, !0), b.setNeonLightEnabled(1, !0), b.setNeonLightEnabled(2, !0), b.setNeonLightEnabled(3, !0)), a.mods.forEach(a => {
                    if (55 == a[0]) b.setWindowTint(a[1] + 1);
                    else if (22 == a[0]) b.setLights(2), 0 > a[1] ? (b.toggleMod(22, !1), mp.game.invoke("0xE41033B25D003A07", b.handle, 255)) : (b.toggleMod(22, !0), mp.game.invoke("0xE41033B25D003A07", b.handle, a[1]));
                    else if (11 === a[0]) switch (a[1]) {
                        case 0:
                            f = 2;
                            break;
                        case 1:
