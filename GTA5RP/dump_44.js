{
  ...
  (global.isSmartphoneOpen = !1),
    (global.smartphoneBrowser = global.mainBrowser),
    global.binder.register({
      ...
      func: () => {
        mp.gui.cursor.visible ||
          ...
          !global.actionAntiFlood("client_smartphone_taskOpen", 2e3) ||
          mp.events.call("client_smartphone_taskOpen");
      },
    }),
    global.binder.register({
      ...
      func: () => {
        global.isSmartphoneOpen &&
          (global.actionAntiFlood("client_smartphone_taskOpen", 2e3),
          global.smartphoneBrowser.execute("___closeSmarphone()"));
      },
    }),
    mp.events.add("client_smartphone_taskOpen", () => {
      localPlayer.getVariable("cuffed") ||
        ...
        mp.events.callRemote("server_smartphone_taskOpen");
    }),
    ...
    mp.events.addDataHandler("characterId", (a, b) => {
      a === localPlayer &&
        global.smartphoneBrowser.execute(`myCharacterId = ${b};`);
    });
  ...
  class PhoneScenarioBase extends global.CustomScenario {
    ...
    async onStart(a) {
      if (a === localPlayer)
        return void (
          ...
          mp.events.call("client_smartphone_open"))
        );
      ...
      mp.players.exists(a) &&
        0 !== a.handle &&
        (mp.attachmentMngr.addClient(a, mp.game.joaat("playerSmartPhone")),
        a.taskPlayAnim(
          "cellphone@str",
    ...
    onEnd(a) {
      return a === localPlayer
        ? void (
            isPlayerHasAnyPhoneScenario(localPlayer) ||
            (mp.game.invoke("0x3BC861DF703E5097"),
            mp.events.call("client_smartphone_close"))
          )
        : void (mp.attachmentMngr.removeFor(
            a,
            mp.game.joaat("playerSmartPhone")
          ),
          a.vehicle
  ...
  mp.events.add("client_smartphone_sync_anim", async (a, b) => {
    ...
    if (mp.players.exists(c) && 0 !== c.handle && c !== localPlayer)
      if (0 === b) {
        ...
        if (!mp.game.streaming.hasAnimDictLoaded("cellphone@self")) {
          ...
          do await mp.game.waitAsync(10);
          while (!mp.game.streaming.hasAnimDictLoaded("cellphone@self"));
        }
        if (!mp.players.exists(c) || 0 === c.handle) return;
        c.taskPlayAnim("cellphone@self", "selfie", 4, 4, -1, 49, 0, !1, !1, !1);
      }
  }),
    (global.smartphoneCallPlayerId = -1),
    mp.events.add("client_smartphone_callFrom", function (a, b) {
      global.smartphoneBrowser.execute(
        `smartphoneAppVue.openApp(phoneAppCall, { callFrom: "${a}", id: "${b}" });`
      );
    }),
    mp.events.add("client_smartphone_call_newStatus", function (a, b) {
      1 === a
        ? ((global.smartphoneCallPlayerId = b),
          global.discordUpdate(
            "\u0413\u043E\u0432\u043E\u0440\u0438\u0442 \u043F\u043E \u0442\u0435\u043B\u0435\u0444\u043E\u043D\u0443"
          ))
        : ((global.smartphoneCallPlayerId = -1), global.discordUpdate()),
        global.smartphoneBrowser.execute(
          `appPhoneCallStore.state.status = ${a};`
        );
    }),
    ...
    mp.events.add("client_smartphone_startCallTo", function (a) {
      global.smartphoneBrowser.execute(
        `smartphoneAppVue.openAppAndPhone(phoneApp, '${a}');`
      );
    }),
    mp.events.add("c:smartphone:sms", (a, b, c, d) => {
      global.smartphoneBrowser.call(
        "b:smartphone:sms",
        a,
        b,
        c ||
    ...
    global.rpc.register("client_smartphone_sms_getPos", function (a) {
      ...
      for (let c = d; 0 != mp.game.invoke("0xA6DB27D19ECBB7DA", c); c = e)
        if (4 == mp.game.invoke("0xBE9B0959FFD0779B", c) && !!b)
          return mp.game.ui.getBlipInfoIdCoord(c);
      return { x: 0, y: 0, z: 0 };
    }),
    mp.events.add("client_smartphone_startSmsTo", function (a) {
      global.smartphoneBrowser.execute(
        `smartphoneAppVue.openAppAndPhone(phoneAppMessanger, '${a}');`
      );
    }),
    mp.events.add("client_smartphone_bankNotify", function (a) {
      0 < a
        ? global.mainBrowser.execute(
            `UI_notify_send("bank+", "+ " + serverAPI.currencyFormatSpace(${a}) + "$");`
          )
        : global.mainBrowser.execute(
            `UI_notify_send("bank-", "- " + serverAPI.currencyFormatSpace(${Math.abs(
              a
            )}) + "$");`
          );
    });
  ...
  function setGPSToPoint(a, b, c, d) {
    isGPSEnable && disableGPS(),
      (isGPSEnable = !0),
      (gpsBlip = mp.blips.new(162, new mp.Vector3(b, c, d), {
        name: `GPS ( ${a} )`,
        color: 3,
        scale: 1.3,
        shortRange: !1,
      })),
      (gpsPos = new mp.Vector3(b, c, d)),
      (gpsInterval = setInterval(() => {
        const { x: a, y: b, z: c } = localPlayer.position,
          d = mp.game.pathfind.calculateTravelDistanceBetweenPoints(
            gpsPos.x,
            gpsPos.y,
            gpsPos.z,
            a,
      ...
      gpsBlip.setRoute(!0),
      gpsBlip.setRouteColour(3);
    const { x: e, y: f, z: g } = localPlayer.position,
      h = mp.game.pathfind.getStreetNameAtCoord(b, c, d, 0, 0),
      i = mp.game.ui.getStreetNameFromHashKey(h.streetName);
    global.smartphoneBrowser.execute(`
        gpsAppStore.state.enable = true;
        gpsAppStore.state.gpsName = '${a}';
        gpsAppStore.state.gpsStreet = '${i}';
        gpsAppStore.state.gpsDist = ${mp.game.pathfind.calculateTravelDistanceBetweenPoints(
          b,
          c,
          d,
          e,
  ...
  function disableGPS() {
    (isGPSEnable = !1),
      ...
      null != gpsBlip &&
        mp.blips.exists(gpsBlip) &&
        (gpsBlip.setRoute(!1), gpsBlip.destroy()),
      (gpsBlip = null),
      global.smartphoneBrowser.execute(`gpsAppStore.state.enable = false;`);
  }
  mp.events.add("client_smartphone_gps_start", setGPSToPoint),
    mp.events.add("client_smartphone_gps_off", disableGPS);
  const raceAppStartRace = async (a, b, c, d) => {
    var e = Math.pow;
    let f = 1,
      g = null;
    const h = () => {
        if (global.isPlayerDeath || 0 !== localPlayer.dimension)
          return (
            i(), void mp.events.callRemote("server_raceApp_endRace", a, !1)
          );
        const { x: b, y: c, z: h } = localPlayer.position,
          j = d[f];
        g ||
          ((g = mp.blips.new(
            f === d.length - 1 ? 38 : 270,
            new mp.Vector3(j[0], j[1], -10),
            { shortRange: !1, name: "", color: 64, dimension: 0 }
          )),
          g.setRoute(!0),
          g.setRouteColour(64)),
          ...
          12 > Math.sqrt(e(j[0] - b, 2) + e(j[1] - c, 2)) &&
            (++f === d.length
              ? (mp.events.callRemote("server_raceApp_endRace", a, !0), i())
              : (g.destroy(), (g = null)));
      },
      i = () => {
        mp.events.remove("render", h), g && g.destroy();
      };
    mp.events.add("render", h),
      ...
      global.resetCamera(),
      global.mainBrowser.execute("startGameCounterSec(5)");
    const j = () => {
    ...
    for (const e of c) {
      ...
      !a ||
        ...
        !a.vehicle ||
        0 === a.vehicle.handle ||
        (a.vehicle.setBurnout(!0),
        a.vehicle.setWheelsCanBreak(!1),
        l.push(a.vehicle));
    }
    await mp.game.waitAsync(4900),
      mp.events.remove("render", j),
      mp.vehicles.exists(k) &&
        0 !== k.handle &&
        (k.setWheelsCanBreak(!0), k.setBurnout(!1)),
      l.forEach(
        (a) =>
          mp.vehicles.exists(a) &&
          0 !== a.handle &&
          (a.setBurnout(!1) || a.setWheelsCanBreak(!0))
      );
  };
  mp.events.add("client_raceApp_startRace", (a) => {
    ...
    raceAppStartRace(
      ...
      b[3]
        .split("|")
        .map((a) => [parseFloat(a.split("%")[0]), parseFloat(a.split("%")[1])])
    );
  }),
    ...
    mp.events.add("clent_raceApp_join", (a, b) => {
      global.smartphoneBrowser.execute(
        `smartphone_raceApp_sync_join(${a}, '${b}')`
      );
    }),
    ...
    mp.events.add("client_raceApp_checkPlayers", (a, b, c) => {
      ...
      for (const e of JSON.parse(c)) {
        ...
        mp.players.forEachInStreamRange((f) => {
          c ||
            (f.getVariable("characterId") === e &&
              50 > Math.sqrt(d(a - f.position.x, 2) + d(b - f.position.y, 2)) &&
              (c = !0));
        }),
  ...
  mp.events.add("client_raceApp_blips", (a) => {
    raceAppBlips.forEach((a) => mp.blips.exists(a) && a.destroy()),
      (raceAppBlips = []),
      null != raceAppBlipsTimeout && clearTimeout(raceAppBlipsTimeout),
      (raceAppBlipsTimeout = null);
    for (const b of JSON.parse(a))
      raceAppBlips.push(
        mp.blips.new(0 === b[2] ? 488 : 490, new mp.Vector3(b[0], b[1], 0), {
          color: 1,
          shortRange: !1,
          dimension: 0,
          scale: 1,
    ...
    mp.api.server.register("client_raceApp_checkStart", (a) => {
      ...
      if (!localPlayer.vehicle)
        return mp.api.server.error(
          "\u0412\u044B \u0434\u043E\u043B\u0436\u043D\u044B \u0431\u044B\u0442\u044C \u0432 \u043C\u0430\u0448\u0438\u043D\u0435"
        );
      const c = (a) => {
          let b = null;
          return (
        ...
        { x: f, y: g, z: h } = localPlayer.position,
      ...
      for (const b of a) {
        ...
        if (
          !a ||
          50 < mp.game.system.vdist(f, g, 0, a.position.x, a.position.y, 0)
        )
          return mp.api.server.error(`${b.name} слишком далеко от вас`);
        if (null == a.vehicle)
          return mp.api.server.error(`${b.name} должен быть в машине`);
        if (
          !mp.game.pathfind.isPointOnRoad(
            a.vehicle.position.x,
            a.vehicle.position.y,
            a.vehicle.position.z,
            0
          )
        )
          return mp.api.server.error(`${b.name} должен быть на дороге`);
        if (15 < Math.abs(i - a.vehicle.getHeading()))
          return mp.api.server.error(
            `${b.name} должен смотреть туда же, куда и вы`
          );
        const h = d(a.vehicle),
          k = e(j, h),
          ...
          m = mp.game.object.getObjectOffsetFromCoords(
            j.x,
            j.y,
            j.z,
            i + 0,
        ...
        if (0.5 < n && 0.5 < o)
          return mp.api.server.error(
            `${b.name} должен стоять с вами на одной линии`
          );
      }
      return mp.api.server.success();
    });
  const vehicleNeonApp = { r: -1, g: -1, b: -1, lightColor: -1, timer: null };
  mp.events.add("clent_smartphone_neon_set", (a, c, d, e, f, g, b) => {
    d &&
      ((vehicleNeonApp.r = e),
      (vehicleNeonApp.g = f),
      (vehicleNeonApp.b = g),
      (vehicleNeonApp.lightColor = b));
    const h =
        vehicleNeonApp.r !== e ||
        vehicleNeonApp.g !== f ||
        vehicleNeonApp.b !== g,
      i = vehicleNeonApp.lightColor !== b;
    null !== vehicleNeonApp.timer &&
      (clearTimeout(vehicleNeonApp.timer), (vehicleNeonApp.timer = null));
    const j = mp.vehicles.atRemoteId(c),
      { x: k, y: l, z: m } = localPlayer.position;
    j &&
      0 !== j.handle &&
      !(25 < mp.dist(j.position.x, j.position.y, j.position.z, k, l, m)) &&
      (j.getIsEngineRunning() || (j.setEngineOn(!0, !0, !0), j.setLights(0)),
      h &&
        (j.setNeonLightEnabled(0, !0),
        j.setNeonLightEnabled(1, !0),
        j.setNeonLightEnabled(2, !0),
        j.setNeonLightEnabled(3, !0),
        j.setNeonLightsColour(e, f, g)),
      i &&
        (255 === b
          ? j.toggleMod(22, !1)
          : (j.toggleMod(22, !0),
            mp.game.invoke("0xE41033B25D003A07", j.handle, b))),
      (h || i) &&
        (vehicleNeonApp.timer = setTimeout(() => {
          (vehicleNeonApp.timer = null),
            (vehicleNeonApp.r = e),
            (vehicleNeonApp.g = f),
            (vehicleNeonApp.b = g),
            (vehicleNeonApp.lightColor = b),
            h && i
              ? mp.events.callRemote(
                  "server_smartphone_neon_update",
                  a,
                  vehicleNeonApp.r,
                  vehicleNeonApp.g,
                  vehicleNeonApp.b,
                  vehicleNeonApp.lightColor
                )
              : h
              ? mp.events.callRemote(
                  "server_smartphone_neon_update",
    ...
    mp.events.add("__client_smartphone_camera", () => {
      let a = !0,
        b = !1,
        c = !1,
        d = !1,
        e = localPlayer.getHealth(),
        f = 0,
        g = 1,
        h = 0,
        i = 0,
      ...
      let v = 0,
        w = 0;
      const x = (a, b, c) => Math.min(c, Math.max(b, a));
      let y = !0;
      const z = new global.Scaleform("instructional_buttons"),
        A = () => {
          z.callFunction("CLEAR_ALL"),
            z.callFunction("TOGGLE_MOUSE_BUTTONS", 0),
            z.callFunction("CREATE_CONTAINER"),
            z.callFunction("SET_CLEAR_SPACE", 200),
            z.callFunction(
              "SET_DATA_SLOT",
              0,
              "w_Enter",
            ...
            z.callFunction(
              "SET_DATA_SLOT",
              1,
              mp.game.controls.getControlActionName(0, 200, !0),
              "\u0412\u044B\u0445\u043E\u0434"
            ),
            b
              ? (z.callFunction(
                  "SET_DATA_SLOT",
                  2,
                  mp.game.controls.getControlActionName(3, 172, !0),
                  "\u0420\u0435\u0436\u0438\u043C"
                ),
                z.callFunction(
                  "SET_DATA_SLOT",
                  ...
                  `${mp.game.controls.getControlActionName(
                    ...
                    87,
                    !0
                  )}%${mp.game.controls.getControlActionName(0, 88, !0)}`,
                  `Фильтр (${o + 1})`
                ),
                z.callFunction(
                  "SET_DATA_SLOT",
                  4,
                  mp.game.controls.getControlActionName(0, 61, !0),
                  p[q].name
                ),
                z.callFunction(
                  "SET_DATA_SLOT",
                  ...
                  `${mp.game.controls.getControlActionName(
                    ...
                    89,
                    !0
                  )}%${mp.game.controls.getControlActionName(0, 90, !0)}`,
                  `Выбор действия`
                ),
                z.callFunction(
                  "SET_DATA_SLOT",
                  ...
                  `${mp.game.controls.getControlActionName(
                    ...
                    51,
                    !0
                  )}%${mp.game.controls.getControlActionName(0, 52, !0)}`,
                  `Эмоция`
                ),
                z.callFunction(
                  "SET_DATA_SLOT",
                  7,
                  mp.game.controls.getControlActionName(0, 0, !0),
                  "\u0411\u043E\u043A\u0435"
                ),
                z.callFunction(
                  "SET_DATA_SLOT",
                  8,
                  mp.game.controls.getControlActionName(0, 69, !0),
                  "\u0420\u0430\u043A\u0443\u0440\u0441"
                ),
                z.callFunction(
                  "SET_DATA_SLOT",
                  9,
                  mp.game.controls.getControlActionName(0, 68, !0),
                  "\u041F\u0440\u043E\u0444\u0438\u043B\u044C"
                ))
              : (z.callFunction(
                  "SET_DATA_SLOT",
                ...
                z.callFunction(
                  ...
                  `${mp.game.controls.getControlActionName(
                    ...
                    180,
                    !0
                  )}%${mp.game.controls.getControlActionName(3, 181, !0)}`,
                  "\u0417\u0443\u043C"
                ),
                z.callFunction(
                  "SET_DATA_SLOT",
            ...
            z.callFunction("DRAW_INSTRUCTIONAL_BUTTONS", -1);
        },
        B = async () => {
          ...
          if (
            ...
            0 !== global.getPlayerCurrentWeaponData().weapon ||
            localPlayer.getVariable("cuffed") ||
            mp.game.controls.isDisabledControlPressed(0, 200)
          )
            return void E(m);
          if (((e = B), mp.keys.isDown(13) && mp.keys.isDown(13)))
            return void (l = !0);
          if (
            (mp.keys.isDown(40) &&
              mp.keys.isDown(40) &&
              global.actionAntiFlood("__smartphoneScaleVisibleChange", 500) &&
              (y = !y),
            mp.game.controls.isControlJustPressed(3, 172) &&
              ...
              a &&
                (mp.game.invoke("0x015C49A93E3E086E", b),
            ...
            if (
              (mp.game.controls.isDisabledControlPressed(0, 69)
                ? (mp.game.controls.disableControlAction(0, 1, !0),
                  ...
                  (g = x(g + b, 0.01, 1.99)),
                  (h = x(
                    h + mp.game.controls.getDisabledControlNormal(0, 30) / 12,
                    -0.99,
                    0.99
                  )))
                : mp.game.controls.isDisabledControlPressed(0, 68) &&
                  ...
                  (i = x(i + a, -0.99, 0.99)),
                  (j = x(
                    j + mp.game.controls.getDisabledControlNormal(0, 30) / 12,
                    -0.99,
                    0.99
                  )),
                  (k = x(k + b, -0.99, 0.99))),
              mp.game.invoke("0x1b0b4aeed5b9b41c", f),
              mp.game.invoke("0x3117d84efa60f77b", g),
              mp.game.invoke("0x15e69e2802c24b8d", h),
              mp.game.invoke("0xd6ade981781fca09", i),
              mp.game.invoke("0xf1e22dc13f5eebad", j),
              mp.game.invoke("0x466da42c89865553", k),
              mp.game.controls.isControlJustPressed(3, 0) &&
                ((d = !d),
                mp.game.invoke(
                  "0xA2CCBE62CD4C91A4",
                  mp.game.invoke("0x375A706A5C2FD084", d)
                )),
              0 == ++w % 15)
            ) {
              const { x: a, y: b, z: c } = localPlayer.position,
          ...
          if (
            ...
            (o += a),
              o >= n.length ? (o = 0) : 0 > o && (o = n.length - 1),
              mp.game.graphics.setTimecycleModifierStrength(1),
              mp.game.graphics.setTimecycleModifier(n[o]),
              A();
          }
          if (
            ...
            (q += a),
              q >= p.length ? (q = 0) : 0 > q && (q = p.length - 1),
              (helpText = `Выбрана анимация - ${p[q].name}`),
              (helpTime = 300),
              A();
          }
          if (
            mp.game.controls.isDisabledControlPressed(0, 51) ||
            mp.game.controls.isDisabledControlPressed(0, 52)
          ) {
            const a = mp.game.controls.isDisabledControlPressed(0, 51) ? -1 : 1;
            (v += a),
              v >= u.length ? (v = 0) : 0 > v && (v = u.length - 1),
              null == u[v].animName
                ? localPlayer.clearFacialIdleAnimOverride()
                : mp.game.invoke(
                    "0xFFC24B988B938B38",
                    localPlayer.handle,
                    u[v].animName,
          ...
          const C = mp.game.controls.isControlPressed(0, 61);
          if (C && !r) {
            const b = p[q];
            if (b) {
              if (((r = !0), !mp.game.streaming.hasAnimDictLoaded(b.dict))) {
                mp.game.streaming.requestAnimDict(b.dict);
                do await mp.game.waitAsync(10);
                while (!mp.game.streaming.hasAnimDictLoaded(b.dict));
              }
              if (!a) return;
              b.anim
                ...
                : (localPlayer.taskPlayAnim(
                  ...
                  await mp.game.waitAsync(
                    1e3 * mp.game.entity.getEntityAnimDuration(b.dict, "enter")
                  ),
                  a &&
                    localPlayer.taskPlayAnim(
                      b.dict,
            ...
            r &&
              ...
              s && !t
                ? (localPlayer.taskPlayAnim(
                  ...
                  await mp.game.waitAsync(
                    1e3 * mp.game.entity.getEntityAnimDuration(s, "exit")
                  ),
                  a &&
                    localPlayer.taskPlayAnim(
                      "",
                ...
                : (localPlayer.stopAnimTask(s, t, 3),
                  localPlayer.taskPlayAnim(
                    "",
                    "",
                    4,
          ...
          if (l) {
            const a = global.getServerDateString(),
              b = global.getServerTime(),
              c = (a) => (10 > a ? "0" + a : a);
            return (
              mp.gui.takeScreenshot(
                `${new Date().getFullYear()}_${a.replace(".", "_")}_${c(
                  b[0]
                )}_${c(b[1])}_${c(b[2])}.png`,
                1,
                100,
              ...
              mp.events.call("client_smartphone_camera_shoot"),
              (m = !0),
              void (l = !1)
            );
          }
        ...
        C = global.uiVisible,
        D = global.targetMenuRender,
        E = (b) => {
          (a = !1),
            global.isSmartphoneOpen &&
              global.smartphoneBrowser.execute(
                `smartphone_cameraApp_end(${b});`
              ),
            C && global.hideUI(!1),
            D && (global.targetMenuRender = !0),
            mp.game.mobile.cellCamActivate(!1, !1),
            localPlayer.setConfigFlag(242, !1),
            localPlayer.setConfigFlag(243, !1),
            localPlayer.setConfigFlag(244, !0),
            mp.game.invoke("0x0F07E7745A236711"),
            (global.disableAnimList = !1),
            mp.events.callRemote("server_smartphone_sync_anim", 0),
            z.destroy(),
            mp.events.remove("render", B);
        };
      C && global.hideUI(!0),
        (global.targetMenuRender = !1),
        A(),
        ...
        localPlayer.setConfigFlag(242, !0),
        localPlayer.setConfigFlag(243, !0),
        localPlayer.setConfigFlag(244, !1),
        (global.disableAnimList = !0),
        mp.events.callRemote("server_smartphone_sync_anim", 1),
    ...
    mp.events.add("client_httpServer_setData", function (a, b, c) {
      global.smartphoneBrowser.execute(
        `smartphone_setHttpServerData("${a}", ${b}, "${c}");`
      );
    }),
    mp.attachmentMngr.register(
      ...
      "p_cs_cam_phone",
      58867,
      new mp.Vector3(0.05, 0.03, 0.01),
      new mp.Vector3(230, 130, 10)
    );
}
