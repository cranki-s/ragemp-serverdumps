...
// Script data
const weaponData = require("general/weaponspawner/weaponData");
...
mp.events.add("playerCommand", (command) => {
    if (command.toLowerCase() === "wep") {
        if (mp.players.local.getVariable('inSandbox') == 1) {
            if (!menusReady) {
                // Create basic menus
                createMainMenu();
                createOptionsMenu();
            ...
            mp.gui.chat.push('[!{255,0,0}Error!{255,255,255}] You need to be in sandbox to do that.');
        }
    }
});
}