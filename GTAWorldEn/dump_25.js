{
﻿const ScaleForm = require("gtalife/ScaleForms.js");
...
mp.events.add("toggle_camera", async (toggle, isSmart) => {
    ...
    if (toggle) {
        ...
        camPos.z = Player.getWorldPositionOfBone(12844).z + 0.65;

        // Create the camera and set it active
        camObj = mp.cameras.new('default', camPos, new mp.Vector3(0, 0, Player.getHeading()), 60);
        camObj.setActive(true);
        mp.game.cam.renderScriptCams(true, false, 0, true, false);
        camObj.attachTo(Player.handle, playerPos.x - camPos.x, playerPos.y - camPos.y, camPos.z, false);
        ...
        if (cameraScale !== null && cameraScale !== undefined) {
            if (cameraScale.handle != null && mp.game.graphics.hasScaleformMovieLoaded(cameraScale.handle)) {
