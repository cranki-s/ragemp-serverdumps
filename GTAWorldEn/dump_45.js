...
mp.events.add('showPhone', (on, zoom, selfnumb, dailyWeather, weeklyWeather, code) => {
	if (!mp.browsers.exists(phoneCEF))
	{
		...
		if(on == true){
			...
			phoneCEF.execute(`LoadWeather('${dailyWeather}', '${weeklyWeather}');`);
...
mp.events.add('showPhoneEx', (name, zoom, selfnumb, dailyWeather, weeklyWeather, code) => {
	if (!mp.browsers.exists(phoneCEF))
	{
		phoneCEF = mp.browsers.new("package://gtalife/Phone/Phone.html");
...
mp.events.add('showPhoneEx2', (number, zoom, selfnumb, dailyWeather, weeklyWeather, code) => {
	if (!mp.browsers.exists(phoneCEF))
	{
		phoneCEF = mp.browsers.new("package://gtalife/Phone/Phone.html");
...
mp.events.add('muteGroups', () => {
	mp.events.callRemote('MuteGroups'); 
});
...
mp.events.add('incomingCall', (name) => {
	if (phoneCEF != null && mp.browsers.exists(phoneCEF))
	{
		...
		mp.gui.chat.push("Use !{#FFFF00}/phonecursor!{#FFFFFF} (/pc) to activate the cursor to use the phone." );
	}
});
...
mp.events.add('sendSMSTimeOut', () => {
	mp.gui.chat.push("You have to wait !{#FF0000}5 seconds!{#FFFFFF} before sending another text!" );
});

mp.events.add('sendSMSLength', () => {
	mp.gui.chat.push("You have too many !{#FF0000}emoticons!{#FFFFFF} in your message!" );
});
