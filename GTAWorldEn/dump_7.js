...
mp.events.add('showSettings', (staff, imageLink, firstname, lastname) => {
    if (settingsCEF == null && !mp.browsers.exists(settingsCEF)) {
        settingsCEF = mp.browsers.new("package://gtalife/SettingsMenu/index.html");
        isStaff_Settings = staff;
        LoadSettings();
        settingsCEF.execute(`Initialize(${JSON.stringify(Settings)}, ${JSON.stringify(HotkeySettings)}, ${staff}, "${imageLink}", "${firstname}", "${lastname}");`);
