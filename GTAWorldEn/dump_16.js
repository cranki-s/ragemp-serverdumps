...
if (mp.game.invokeString("0x7CE1CCB9B293020E", mp.players.local.vehicle.handle).trim().valueOf() == 'RENTAL'.valueOf())
{
}
else
{
    if(!cruiseEnabled){
        if(mp.players.local.vehicle.getSpeed() > 0) {
            if(Date.now() >= lastCruiseTime+15000){
                ...
                mp.players.local.vehicle.setMaxSpeed(Math.ceil(cruiseSpeed * 0.9));
                mp.game.graphics.notify("~w~Cruise Control turned ~g~ON~w~ at speed ~b~" + (cruiseSpeed * 2.236936).toFixed(0) + " ~w~MPH.");
    ...
    if(setOff && cruiseEnabled){
        ...
        mp.players.local.vehicle.setMaxSpeed(vehMaxSpeed);
        mp.game.graphics.notify("~w~Cruise Control turned ~r~OFF~w~.");
...
mp.events.add('render', () => {
    if (mp.players.local === null || mp.players.local === undefined)
        return;
    mp.game.ui.hideHudComponentThisFrame(9); // Disable the street name
    mp.game.ui.hideHudComponentThisFrame(7); // Disable the area name
    mp.game.ui.hideHudComponentThisFrame(3); // Disable the hand money from singleplayer
    mp.game.ui.hideHudComponentThisFrame(4); // Disable the bank money from singleplayer
    mp.players.local.setHelmet(false);
    if (mp.players.local.isPerformingStealthKill()) mp.players.local.clearTasksImmediately();
    ...
    if (!mp.players.local.isUsingActionMode() && meleeAttack)
    {
        ...
        if (timeout > 0)
        {
            ...
            timeout = setTimeout(() => { timeout = 0; if (!meleeAttack)
	...
	if (logged) {
        ...
        if (draw_text) {
            ...
            drawText(`GTA.WORLD — ~y~${globalBuild}~w~ — ${m_onlineCount}/1500`, [(res_X / 2) / res_X, (res_Y - playerHUD_Y) / res_Y], 4, [255, 255, 255, 200], playerHUD_Scale, true);
        }
        /*if (isInCCTV === true) {
            // CCTV UI
...
function staffDrawText()
{
    const isAdminDuty = mp.players.local.hasVariable('ON_DUTY_ADMIN') && mp.players.local.getVariable('ON_DUTY_ADMIN');
    const isTesterDuty = mp.players.local.hasVariable('ON_DUTY_TESTER') && mp.players.local.getVariable('ON_DUTY_TESTER');
    
    if (isAdminDuty) {
        mp.game.graphics.drawText("Admin-Duty", [(res_X - 70) / res_X, 0.14], {
            font: 4, 
            color: [28, 128, 0, 255],
            outline: true,
            centre: true
    ...
    if (isTesterDuty) {
        mp.game.graphics.drawText("Tester-Duty", [(res_X - 70) / res_X, 0.175], {
            font: 4, 
            color: [115, 16, 16, 255],
            outline: true,
            centre: true
