...
let rouletteCam = false;
let blockControls = false;
let setBet = false;
let curBetId = 0;

let rouletteHud = null;
let inGame = false;

let currentlyPlaced = 0;
mp.events.add('client__rl_hudUpdate', (act, ...data) => {
    switch (act) {
        case 'setUIVisible':
            if (data[0] == true){
                rouletteHud = mp.browsers.new('package://RageServer/Casino/roulette/cef/hud/index.html');
                inGame = false;
            }
            else {
                rouletteHud?.destroy();
            ...
            break;
        case 'updateCounter':
            rouletteHud?.execute(`updateCounter(${data[0]})`);
            break;
        case 'updateBet':
            rouletteHud?.execute(`updateBet(${data[0]})`);
            break;
        case 'updateMoney':
            rouletteHud?.execute(`updateMoney(${data[0]})`);
            break;
        case 'addPlaced':
            currentlyPlaced += data[0];
            rouletteHud?.execute(`updatePlaced(${currentlyPlaced})`);
            break;
        case 'setCounterVisible':
            inGame = !data[0];
            rouletteHud?.execute(`setCounterVisible(${data[0]})`);
            break;
    }
});

let RouletteBetChipModel = {
    10: "vw_prop_chip_10dollar_x1",
    50: "vw_prop_chip_50dollar_x1",
    100: "vw_prop_chip_100dollar_x1",
    500: "vw_prop_chip_50dollar_x1",
    1000: "vw_prop_chip_1kdollar_x1",
    //5000: "vw_prop_chip_5kdollar_x1",
    //10000: "vw_prop_chip_10kdollar_x1"
}

let RoulettePossibleBets = [
    10,
    50,
    100,
    500,
...
let RoulettePeds = [
    {Hash: mp.game.joaat("u_f_m_casinocash_01"), Pos: new mp.Vector3(-1821.5819091796875, -1194.01806640625, 14.307744026184082), Angle: 150},
    {Hash: mp.game.joaat("u_f_m_casinoshop_01"), Pos: new mp.Vector3(-1824.7796630859375, -1199.4437255859375, 14.30774211883545), Angle: -30},
];
...
const RouletteCameraPos = [
    ...
    new mp.Vector3(-1822.3831787109375, -1195.96142578125, 14.307731628417969 + 2.5),
    new mp.Vector3(-1823.9508056640625, -1197.5093994140625, 14.30774211883545 + 2.5),
];
...
const RouletteTablesPos = [
    new mp.Vector3(-1821.938, -1194.629, 13.29744),
    new mp.Vector3(-1824.394, -1198.837, 13.29744),
];
...
setTimeout(function () {
    ...
    RoulettePeds.forEach(ped => {
        RoulettePedsID[n] = mp.peds.new(ped.Hash, ped.Pos, ped.Angle, 0);
        RoulettePedsID[n].setComponentVariation(0, 2, 1, 0);
        RoulettePedsID[n].setComponentVariation(1, 1, 0, 0);
        RoulettePedsID[n].setComponentVariation(2, 2, 0, 0);
        RoulettePedsID[n].setComponentVariation(3, 0, n + 2, 0);
        RoulettePedsID[n].setComponentVariation(4, 0, 0, 0);
        RoulettePedsID[n].setComponentVariation(6, 1, 0, 0);
        RoulettePedsID[n].setComponentVariation(7, 2, 0, 0);
        RoulettePedsID[n].setComponentVariation(8, 1, 0, 0);
        RoulettePedsID[n].setComponentVariation(10, 1, 0, 0);
        RoulettePedsID[n].setComponentVariation(11, 1, 0, 0);
        RoulettePedsID[n].setConfigFlag(185, true);
        RoulettePedsID[n].setConfigFlag(108, true);
        RoulettePedsID[n].setConfigFlag(208, true);
        RoulettePedsID[n].taskPlayAnim("anim_casino_b@amb@casino@games@shared@dealer@", "idle", 1000.0, -2.0, -1, 2, 1148846080, false, false, false);
        n = n + 1;
    });
	n = 0;
}, 1000);
...
mp.events.add('client__rl_spinWheel', function(tb, needSpins, endTable, endBall){
    ...
    RoulettePedsID[tb].taskPlayAnim("anim_casino_b@amb@casino@games@roulette@dealer", "spin_wheel", 8.0, 1, -1, 2, 0.0, false, false, false);

    setTimeout(
		function()
		{
            RoulettePedsID[tb].taskPlayAnim("anim_casino_b@amb@casino@games@roulette@dealer", "idle", 8.0, 1, -1, 1, 0.0, false, false, false);
		}, 8000
	);
});
...
mp.events.add('render', () => {
    ...
    if(setBet && rouletteCamera != null && rouletteTable != null && startRoulette == false){
        if(canDoBets && betObject == null)
        {
            betObject = mp.objects.new(mp.game.joaat(RouletteBetChipModel[RoulettePossibleBets[curBetId]]), new mp.Vector3(RouletteTablesPos[rouletteTable].x, RouletteTablesPos[rouletteTable].y, RouletteTablesPos[rouletteTable].z + 0.4));
            betObject.setCollision(false, false);
        }
        if(betObject != null && canDoBets)
		{
            betObject.model = mp.game.joaat(RouletteBetChipModel[RoulettePossibleBets[curBetId]]);
			if(mp.game.controls.isDisabledControlJustReleased(0, 24))
			{
				if(closestChipSpot != null) mp.events.callRemote("server__rl_make_bet", closestChipSpot);
            }
    ...
    if(startRoulette == true && rouletteTable != null && !rouletteCam){
    ...
    if(startRoulette == false && rouletteTable != null && rouletteCam){
...
mp.events.add('client__rl_cleanChips', function( table){

    RoulettePedsID[table].taskPlayAnim("anim_casino_b@amb@casino@games@roulette@dealer", "clear_chips_intro", 3.0, 1.0, -1, 2, 0, false, false, false);
    ...
    setTimeout(() => {
        RoulettePedsID[table].taskPlayAnim("anim_casino_b@amb@casino@games@roulette@dealer", "clear_chips_outro", 3.0, 1.0, -1, 2, 0, false, false, false);
    }, 1000);
});


mp.events.add('client__rl_start', function(){
    startRoulette = true;
});

mp.events.add('client__rl_stop', function(){
    startRoulette = false;
});

mp.events.add('client__rl_bet', function(table, spot){
    ...
    betRoulette[table].push(mp.objects.new(mp.game.joaat(RouletteBetChipModel[RoulettePossibleBets[curBetId]]), new mp.Vector3(newCardPos.x, newCardPos.y, newCardPos.z),
    {
...
mp.events.add('client__rl_seatToTable', async function(table, seat, player){
    let boneIndex = RouletteTables[table].table.getBoneIndexByName(RouletteSeats[seat]);
    const bonePos = RouletteTables[table].table.getWorldPositionOfBone(boneIndex);
    let boneRot = mp.game.invokeVector3("0xCE6294A232D03786", RouletteTables[table].table.handle, boneIndex);
    ...
    player.setRotation(0, 0, RouletteTablesSeatsHeading[table][seat], 1 ,false);
    ...
    if (player === mp.players.local){
        ...
        curBetId = 0;
        mp.events.callRemote('server__rl_setBet', 10);
    }
...
mp.events.add('client__rl_exit', function(player){
try {
        if(player){
...
mp.events.add('client__rl_win_notification', function(number, amount){
    let moneyDelta = parseInt(amount) - parseInt(currentlyPlaced);
        mp.game.graphics.notify(`Gezogene Nummer: ~y~${number}`);
    if (moneyDelta < 0){
        mp.game.graphics.notify(`Gewonnen: ~g~$ ${amount.toLocaleString('de-DE')}~w~~n~- Einsatz: ~r~$ ${currentlyPlaced.toLocaleString('de-DE')}~w~~n~= Gesamtverlust: ~r~$ ${Math.abs(moneyDelta).toLocaleString('de-DE')}`);
    }else{
        mp.game.graphics.notify(`Gewonnen: ~g~$ ${amount.toLocaleString('de-DE')}~w~~n~- Einsatz: ~r~$ ${currentlyPlaced.toLocaleString('de-DE')}~w~~n~= Gesamtgewinn: ~g~$ ${Math.abs(moneyDelta).toLocaleString('de-DE')}`);
    }
    currentlyPlaced = 0;
});

mp.events.add('client__rl_cannot_bet_notification', function(){
    mp.game.graphics.notify(`~r~Du kannst aktuell nicht wetten!`);
});

mp.events.add('client__rl_not_enough_money_notification', function(){
    mp.game.graphics.notify(`~r~Du hast nicht genug Geld dabei!`);
});

mp.events.add('client__rl_loose_notification', function(number){
    mp.game.graphics.notify(`Gezogene Nummer: ~y~${number}`);
    mp.game.graphics.notify(`Du hast deinen Einsatz von~n~~r~$ ${currentlyPlaced.toLocaleString('de-DE')} ~w~komplett verloren!`);
    currentlyPlaced = 0;
});
...
mp.keys.bind(0x45, false, async function () { // E
    if(canSeat){
        mp.events.callRemote('server__rl_tryPlayerSeat');
        canSeat = false;
...
mp.keys.bind(0x6B, true, () => { // VK_ADD
    if(mp.keys.isDown(0x6B) && rouletteTable !== null && canDoBets){
        if (curBetId < RoulettePossibleBets.length - 1){
            mp.events.callRemote('server__rl_setBet', RoulettePossibleBets[++curBetId]);
        }
    }
});

mp.keys.bind(0x6D, true, () => {  // VK_SUBTRACT
    if(mp.keys.isDown(0x6D) && rouletteTable !== null && canDoBets){
        if (curBetId > 0){
            mp.events.callRemote('server__rl_setBet', RoulettePossibleBets[--curBetId]);
        }
    }
